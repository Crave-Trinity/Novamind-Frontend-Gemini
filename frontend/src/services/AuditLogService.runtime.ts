...
export function isAuditLogEvent(obj: unknown): obj is AuditLogEvent {
    ...
    return (
        ...
        (event.result === 'success' || event.result === 'failure' || event.result === 'warning') &&
        (event.details === undefined || typeof event.details === 'string') &&
        (event.resourceType === undefined || typeof event.resourceType === 'string') &&
        (event.resourceId === undefined || typeof event.resourceId === 'string') &&
        (event.clientInfo === undefined || (typeof event.clientInfo === 'object' && event.clientInfo !== null)) // Basic object check for clientInfo
...
export function isPartialAuditLogEventData(obj: unknown): obj is Partial<AuditLogEvent> {
     if (typeof obj !== 'object' || obj === null) return false;
     // Since it's Partial, we mainly check types if properties exist
     const data = obj as Partial<AuditLogEvent>;

     if (data.timestamp !== undefined && typeof data.timestamp !== 'string') return false;
     if (data.eventType !== undefined && !isAuditEventType(data.eventType)) return false;
     if (data.userId !== undefined && typeof data.userId !== 'string') return false;
     if (data.sessionId !== undefined && typeof data.sessionId !== 'string') return false;
     if (data.result !== undefined && !['success', 'failure', 'warning'].includes(data.result)) return false;
     if (data.details !== undefined && typeof data.details !== 'string') return false;
     if (data.resourceType !== undefined && typeof data.resourceType !== 'string') return false;
     if (data.resourceId !== undefined && typeof data.resourceId !== 'string') return false;
     if (data.clientInfo !== undefined && (typeof data.clientInfo !== 'object' || data.clientInfo === null)) return false;
 ...
 */
export function validateLogEventData(data: unknown): Result<Partial<AuditLogEvent>, Error> {
    if (isPartialAuditLogEventData(data)) {
        return Ok(data);
    }
    return Err(new Error('Invalid data for AuditLogEvent: Structure or types are incorrect.'));
}
 ...
 */
export function validateAuditEvent(event: unknown): Result<AuditLogEvent, Error> {
    ...
    return Err(new Error(`Invalid AuditLogEvent structure. Received: ${eventStr}`));
}
