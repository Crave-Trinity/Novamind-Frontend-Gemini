...
export function isSessionOptions(obj: unknown): obj is SessionOptions {
    ...
    if (options.warningTime !== undefined && (typeof options.warningTime !== 'number' || options.warningTime <= 0)) return false;
    if (options.onTimeout !== undefined && typeof options.onTimeout !== 'function') return false;
    if (options.onWarning !== undefined && typeof options.onWarning !== 'function') return false;
    if (options.enabled !== undefined && typeof options.enabled !== 'boolean') return false;

    // Check that warningTime is less than timeout if both are provided
    if (options.timeout !== undefined && options.warningTime !== undefined && options.warningTime >= options.timeout) {
        console.warn("SessionOptions validation: warningTime should be less than timeout.");
 ...
 */
export function validateSessionOptions(options: unknown): Result<SessionOptions, Error> {
    ...
    return Err(new Error(`Invalid SessionOptions structure or types. Received: ${optionsStr}`));
}
