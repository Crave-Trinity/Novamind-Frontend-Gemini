...
export const brainModelService = {
  ...
  updateRegion: async (
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            ...
            case 403:
              return failure(
                new Error("Insufficient permissions to update this region"),
              );
            case 500:
      ...
      return failure(
        new Error(
          `Failed to update region: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
    }
  },
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            ...
            case 403:
              return failure(
                new Error("Insufficient permissions to update this connection"),
              );
            case 500:
              return failure(
                new Error("Server error while updating connection"),
              );
            default:
              return failure(new Error(data.message || `API error: ${status}`));
          }
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            ...
            case 403:
              return failure(
                new Error("Insufficient permissions to create annotations"),
              );
            case 500:
              return failure(
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            ...
            case 400:
              return failure(
                new Error(`Invalid request data: ${data.message}`),
              );
            case 403:
              return failure(
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            case 404:
              return failure(
                new Error(`Generation task with ID ${generationId} not found`),
              );
            case 403:
              return failure(
                new Error(
                  "Insufficient permissions to check this generation task",
                ),
              );
            case 500:
              return failure(
