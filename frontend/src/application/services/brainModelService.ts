...
export const brainModelService = {
  ...
  fetchBrainModel: async (scanId: string): Promise<Result<BrainModel>> => {
    try {
      ...
      return failure(new Error(`Failed to fetch brain model: ${error instanceof Error ? error.message : String(error)}`));
    }
  },
  ...
  searchBrainModels: async (
    ...
    try {
      ...
      // API request
      const response = await axios.get<{ data: BrainModel[]; total: number }>(BRAIN_MODEL_ENDPOINT, {
        params,
      ...
      // Generic error handling
      return failure(new Error(`Failed to search brain models: ${error instanceof Error ? error.message : String(error)}`));
    }
  },
  ...
  updateRegion: async (
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            ...
            case 403:
              return failure(new Error('Insufficient permissions to update this region'));
            case 500:
              return failure(new Error('Server error while updating region'));
            default:
              return failure(new Error(data.message || `API error: ${status}`));
          }
        } else if (error.request) {
      ...
      // Generic error handling
      return failure(new Error(`Failed to update region: ${error instanceof Error ? error.message : String(error)}`));
    }
  },
  ...
  updateConnection: async (
    ...
    try {
        ...
        if (error.response) {
          ...
          switch (status) {
            ...
            case 400:
              return failure(new Error(`Invalid update data: ${data.message}`));
            case 403:
              return failure(new Error('Insufficient permissions to update this connection'));
            case 500:
              return failure(new Error('Server error while updating connection'));
            default:
              return failure(new Error(data.message || `API error: ${status}`));
          }
        } else if (error.request) {
      ...
      // Generic error handling
      return failure(new Error(`Failed to update connection: ${error instanceof Error ? error.message : String(error)}`));
    }
  },
  ...
  createAnnotation: async (
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            ...
            case 403:
              return failure(new Error('Insufficient permissions to create annotations'));
            case 500:
      ...
      // Generic error handling
      return failure(new Error(`Failed to create annotation: ${error instanceof Error ? error.message : String(error)}`));
    }
  },
  ...
  generateModel: async (patientId: string): Promise<Result<{ scanId: string; status: string }>> => {
    try {
      // Mock implementation - in a real system, this would be a POST request to trigger model generation
      // API request for model generation
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            ...
            case 400:
              return failure(new Error(`Invalid request data: ${data.message}`));
            case 403:
      ...
      // Generic error handling
      return failure(new Error(`Failed to generate brain model: ${error instanceof Error ? error.message : String(error)}`));
    }
  },
   ...
   */
  checkGenerationStatus: async (generationId: string): Promise<Result<{ status: string; progress: number; scanId?: string; error?: string }>> => {
    try {
      // API request to check generation status
      const response = await axios.get<{ status: string; progress: number; scanId?: string; error?: string }>(
        `${BRAIN_MODEL_ENDPOINT}/generate/${generationId}`,
        {
          timeout: 10000,
          headers: {
            'Accept': 'application/json',
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          switch (status) {
            case 404:
              return failure(new Error(`Generation task with ID ${generationId} not found`));
            case 403:
              return failure(new Error('Insufficient permissions to check this generation task'));
            case 500:
      ...
      // Generic error handling
      return failure(new Error(`Failed to check generation status: ${error instanceof Error ? error.message : String(error)}`));
    }
  }
};
