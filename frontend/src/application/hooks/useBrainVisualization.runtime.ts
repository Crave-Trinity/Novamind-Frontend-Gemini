...
export function validatePartialBrainViewState(data: unknown): Result<PartialBrainViewState, Error> {
  ...
  if (viewState.highlightedRegions !== undefined && (!Array.isArray(viewState.highlightedRegions) || !viewState.highlightedRegions.every(id => typeof id === 'string'))) {
     return Err(new Error('Invalid PartialBrainViewState: highlightedRegions must be an array of strings.'));
  }
  ...
  if (viewState.renderMode !== undefined && !isValidRenderMode(viewState.renderMode)) {
     return Err(new Error(`Invalid PartialBrainViewState: Invalid renderMode value "${viewState.renderMode}".`));
  }
   if (viewState.transparencyLevel !== undefined && (typeof viewState.transparencyLevel !== 'number' || viewState.transparencyLevel < 0 || viewState.transparencyLevel > 1)) {
     return Err(new Error('Invalid PartialBrainViewState: transparencyLevel must be a number between 0 and 1.'));
  }
   if (viewState.focusPoint !== undefined && viewState.focusPoint !== null && (!Array.isArray(viewState.focusPoint) || viewState.focusPoint.length !== 3 || !viewState.focusPoint.every(n => typeof n === 'number'))) {
     return Err(new Error('Invalid PartialBrainViewState: focusPoint must be null or an array of three numbers.'));
  }
