...
const createValidMockPatient = (): Patient => ({
  id: 'patient-123',
  demographicData: { // Required
    age: 35,
    biologicalSex: 'female',
    anonymizationLevel: 'clinical',
  },
  clinicalData: { // Required
    diagnoses: [],
    symptoms: [],
    medications: [],
  ...
  dataAccessPermissions: { // Required
    accessLevel: 'full',
...
describe('usePatientData Runtime Validation', () => {
  describe('validatePatientData', () => {
    ...
    it('should return Err for non-object input', () => {
      ...
      expect((result.val as Error).message).toContain('Invalid PatientData: Data does not conform to the Patient structure.');
    });
    ...
    it('should return Err for data missing required top-level fields (e.g., id)', () => {
      const invalidData = { // Missing 'id' and others
        demographicData: { age: 40, biologicalSex: 'male', anonymizationLevel: 'research' },
        ...
        dataAccessPermissions: { accessLevel: 'research', authorizedUsers: [], consentStatus: 'research-only', dataRetentionPolicy: 'research', lastReviewDate: '' },
        lastUpdated: '', version: ''
      };
      const result = validatePatientData(invalidData);
      expect(result.err).toBe(true);
    ...
    it('should return Err for data with incorrect field types (e.g., id is number)', () => {
     ...
     it('should return Err for data with incorrect nested field types (e.g., clinicalData.diagnoses is not array)', () => {
      const invalidData = {
        ...createValidMockPatient(),
        clinicalData: { ...createValidMockPatient().clinicalData, diagnoses: 'not-an-array' },
      };
      const result = validatePatientData(invalidData);
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('Invalid PatientData: Data does not conform to the Patient structure.');
