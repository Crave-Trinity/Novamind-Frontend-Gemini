...
import { TreatmentResponsePrediction, isTreatmentResponsePrediction } from '@domain/types/clinical/treatment';
import {
 ...
 */
export function validateTreatmentResponsePredictionArray(data: unknown): Result<TreatmentResponsePrediction[], Error> {
  ...
  if (!data.every(isTreatmentResponsePrediction)) {
     return Err(new Error('Invalid TreatmentResponsePrediction array: One or more items do not conform to the TreatmentResponsePrediction structure.'));
  }
...
export function validateSymptomMappingArray(data: unknown): Result<SymptomNeuralMapping[], Error> {
  ...
  if (!data.every(isSymptomNeuralMapping)) {
     return Err(new Error('Invalid SymptomNeuralMapping array: One or more items do not conform to the SymptomNeuralMapping structure.'));
  }
  return Ok(data); // Type assertion is safe due to .every check
}
 ...
 */
export function validateDiagnosisMappingArray(data: unknown): Result<DiagnosisNeuralMapping[], Error> {
  if (!Array.isArray(data)) {
    return Err(new Error('Invalid DiagnosisNeuralMapping array: Input must be an array.'));
  }
   if (!data.every(isDiagnosisNeuralMapping)) {
     return Err(new Error('Invalid DiagnosisNeuralMapping array: One or more items do not conform to the DiagnosisNeuralMapping structure.'));
  }
  return Ok(data); // Type assertion is safe due to .every check
}
 ...
 */
export function validateTreatmentMappingArray(data: unknown): Result<TreatmentNeuralMapping[], Error> {
  if (!Array.isArray(data)) {
    return Err(new Error('Invalid TreatmentNeuralMapping array: Input must be an array.'));
  }
   if (!data.every(isTreatmentNeuralMapping)) {
     return Err(new Error('Invalid TreatmentNeuralMapping array: One or more items do not conform to the TreatmentNeuralMapping structure.'));
  }
  return Ok(data); // Type assertion is safe due to .every check
}
