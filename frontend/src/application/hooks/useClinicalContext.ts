...
export function useClinicalContext(patientId?: string): UseClinicalContextReturn {
  ...
  const {
    ...
    async () => {
      ...
      if (result.success) {
        ...
        throw result.error || new Error('Failed to fetch treatment predictions');
      }
    },
    {
      enabled: !!patientId,
  ...
  const refreshClinicalData = useCallback(async (patientId: string) => {
    await Promise.all([
  ...
  const fetchSymptomMappings = useCallback(async (): Promise<Result<SymptomNeuralMapping[]>> => {
    try {
      ...
      if (result.success) {
        ...
        return failure(result.error || new Error('Failed to fetch symptom mappings'));
      }
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Unknown error fetching symptom mappings');
      return failure(error);
    }
  }, [queryClient]);
  
  const fetchDiagnosisMappings = useCallback(async (): Promise<Result<DiagnosisNeuralMapping[]>> => {
    try {
      const result = await clinicalService.fetchDiagnosisMappings();
      ...
      if (result.success) {
        ...
        return failure(result.error || new Error('Failed to fetch diagnosis mappings'));
      }
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Unknown error fetching diagnosis mappings');
      return failure(error);
    }
  }, [queryClient]);
  
  const fetchTreatmentMappings = useCallback(async (): Promise<Result<TreatmentNeuralMapping[]>> => {
    try {
      const result = await clinicalService.fetchTreatmentMappings();
      ...
      if (result.success) {
        ...
        return failure(result.error || new Error('Failed to fetch treatment mappings'));
      }
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Unknown error fetching treatment mappings');
      return failure(error);
    }
  }, [queryClient]);
  
  const fetchRiskAssessment = useCallback(async (patientId: string): Promise<Result<RiskAssessment>> => {
    try {
      const result = await clinicalService.fetchRiskAssessment(patientId);
      ...
      if (result.success) {
        ...
        return failure(result.error || new Error('Failed to fetch risk assessment'));
      }
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Unknown error fetching risk assessment');
      return failure(error);
    }
  }, [queryClient]);
  
  const fetchTreatmentPredictions = useCallback(async (patientId: string): Promise<Result<TreatmentResponsePrediction[]>> => {
    try {
      const result = await clinicalService.fetchTreatmentPredictions(patientId);
      
      if (result.success) {
        queryClient.setQueryData([treatmentPredictionsKey, patientId], result.data);
        return success(result.data);
      } else {
        return failure(result.error || new Error('Failed to fetch treatment predictions'));
      }
    } catch (err) {
      const error = err instanceof Error ? err : new Error('Unknown error fetching treatment predictions');
      return failure(error);
    }
  }, [queryClient]);
