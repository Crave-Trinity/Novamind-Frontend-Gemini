...
export function useVisualSettings(): UseVisualSettingsReturn {
  ...
  const [localSettings, setLocalSettings] = useState<VisualizationSettings>(DEFAULT_VISUALIZATION_SETTINGS);
  
  // Local state for theme settings
  const [localThemeSettings, setLocalThemeSettings] = useState<Record<string, ThemeSettings>>(DEFAULT_THEME_SETTINGS);
  ...
  useEffect(() => {
    try {
      ...
      if (storedThemeSettings) {
        const parsedThemeSettings = JSON.parse(storedThemeSettings) as Record<string, ThemeSettings>;
  ...
  const getThemeSettings = useCallback((themeName: string): ThemeSettings => {
