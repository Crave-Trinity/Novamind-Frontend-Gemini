...
export function useBrainModel(): UseBrainModelReturn {
  ...
  const fetchBrainModel = useCallback(async (scanId: string): Promise<Result<BrainModel>> => {
    try {
  ...
  >(
    async (regionId) => {
      ...
      } else if (!isActive && updatedModel.regions[regionIndex].activityLevel > 0.3) {
  ...
  const updateRegionActivity = useCallback((regionId: string, activityLevel: number) => {
    updateRegionActivityMutation.mutate({ regionId, activityLevel });
  }, [updateRegionActivityMutation]);
  
  // Toggle region active
  const toggleRegionActive = useCallback((regionId: string) => {
  ...
  // Combine errors
  const error = queryError || updateRegionActivityMutation.error || toggleRegionActiveMutation.error;
  ...
  return {
    ...
    // State
    isLoading: isLoading || updateRegionActivityMutation.isLoading || toggleRegionActiveMutation.isLoading,
