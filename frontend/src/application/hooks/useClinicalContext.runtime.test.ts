...
const mockValidRiskAssessment: RiskAssessment = {
  id: 'ra-1', patientId: 'p1', timestamp: new Date().toISOString(), assessmentType: 'hybrid',
  overallRisk: RiskLevel.MODERATE, confidenceScore: 0.8, domainRisks: [],
  temporalTrend: 'stable', contributingFactors: [], protectiveFactors: [], neuralCorrelates: [],
};
...
const mockValidTreatmentPrediction: TreatmentResponsePrediction = {
  requestId: 'req-1', patientId: 'p1', treatmentType: 'pharmacological', timestamp: new Date().toISOString(), // Use string literal
  algorithm: { name: 'XGBoost', version: '1.2', confidence: 0.9 },
  prediction: {
    responseType: 'response', responseProbability: 0.75, confidenceInterval: [0.6, 0.9],
    timeToEffect: { expected: 14, range: [7, 21] }, durability: { expected: 6, probability: 0.8 }
  },
  symptomSpecificPredictions: [], sideEffectRisks: [], neurobiologicalMechanisms: [],
  personalizationFactors: [], // Add missing required property
  limitations: [], alternatives: [], dataQualityAssessment: { overallQuality: 'high', missingDataImpact: 'minimal', biasRiskLevel: 'low' }
};
...
const mockValidNeuralPattern: NeuralActivationPattern = {
  regionIds: ['r1', 'r2'], intensity: 0.7, confidence: 0.9, timeScale: 'acute',
  connectivity: { increasedPathways: [], decreasedPathways: [] }
};

const mockValidSymptomMapping: SymptomNeuralMapping = {
  symptomId: 's1', symptomName: 'Anxiety', category: 'Emotional', activationPatterns: [mockValidNeuralPattern],
  contributingFactors: [], evidenceQuality: 'established'
};

const mockInvalidSymptomMapping = { symptomId: 's2' }; // Missing fields

const mockValidDiagnosisMapping: DiagnosisNeuralMapping = {
  diagnosisId: 'd1', diagnosisName: 'MDD', codingSystem: 'ICD-10', activationPatterns: [mockValidNeuralPattern],
...
const mockValidTreatmentMapping: TreatmentNeuralMapping = {
  treatmentId: 't1', treatmentName: 'SSRI', treatmentType: 'pharmacological', // Use string literal
  mechanismsOfAction: [], effectPatterns: { increasedActivity: [], decreasedActivity: [], normalizedConnectivity: [] },
