...
const mockValidPartialConfig: any = {
...
describe("TemporalDynamicsController Runtime Validation", () => {
  ...
  describe("validatePartialTemporalConfig", () => {
    ...
    it("should return Err for config with incorrect field types", () => {
      const result = validatePartialTemporalConfig(
        mockInvalidPartialConfig_WrongType,
      );
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain(
        "patternRecognitionThreshold must be a number",
      );
    });

    it("should return Err for config with out-of-range values", () => {
      const result = validatePartialTemporalConfig(
        mockInvalidPartialConfig_OutOfRange,
      );
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain(
        "smoothingFactor must be a number between 0.0 and 1.0.",
      );
    });

    it("should return Err for config with invalid historyLength keys", () => {
      const result = validatePartialTemporalConfig(
        mockInvalidPartialConfig_BadRecordKey,
      );
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain(
        "historyLength must be a Record mapping TimeScale",
      );
    });

    it("should return Err for config with invalid samplingRate values", () => {
      const result = validatePartialTemporalConfig(
        mockInvalidPartialConfig_BadRecordValue,
      );
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain(
        "samplingRate must be a Record mapping TimeScale to positive numbers.",
      );
    });
  });
