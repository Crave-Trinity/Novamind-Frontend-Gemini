...
const mockValidPartialStreamConfig: any = { // Use 'any' for partial mock flexibility
  sampleRate: 120,
...
const mockInvalidPartialStreamConfig_InvalidThreshold = {
  alertThresholds: new Map([
      ['heartRate', [{ min: 'low', max: 100, label: 'Warn', priority: 'warning' }]] // min is not number
  ])
};

const mockInvalidPartialStreamConfig_InvalidThresholdKey = {
  alertThresholds: new Map([
      ['heart_rate_bpm', [{ min: 50, max: 100, label: 'Warn', priority: 'warning' }]] // Invalid BiometricType key
  ])
};
...
describe('BiometricStreamController Runtime Validation', () => {
  ...
  describe('validateBiometricData', () => {
    ...
    it('should return Err for non-object input', () => {
      ...
      expect((result.val as Error).message).toContain('Invalid BiometricDataPoint structure.');
    });

     it('should return Err for data missing required fields', () => {
      const result = validateBiometricData(mockInvalidBiometricDataPoint_MissingField);
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('Invalid BiometricDataPoint structure.');
    });

     it('should return Err for data with incorrect field types', () => {
      const result = validateBiometricData(mockInvalidBiometricDataPoint_WrongType);
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('Invalid BiometricDataPoint structure.');
    });
  });
  ...
  describe('validatePartialStreamConfig', () => {
    ...
    it('should return Err for non-object input', () => {
     ...
     it('should return Err for config with incorrect field types', () => {
      const result = validatePartialStreamConfig(mockInvalidPartialStreamConfig_WrongType);
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('sampleRate must be a positive number.');
    });

     it('should return Err for config with invalid alertThresholds structure (invalid threshold)', () => {
      const result = validatePartialStreamConfig(mockInvalidPartialStreamConfig_InvalidThreshold);
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('Invalid threshold array for key "heartRate"');
    });

     it('should return Err for config with invalid alertThresholds structure (invalid key)', () => {
      const result = validatePartialStreamConfig(mockInvalidPartialStreamConfig_InvalidThresholdKey);
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('Invalid key "heart_rate_bpm"');
    });

     it('should return Err for config with invalid sources array', () => {
      const result = validatePartialStreamConfig(mockInvalidPartialStreamConfig_InvalidSource);
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('sources must be an array of valid BiometricSource values.');
    });
