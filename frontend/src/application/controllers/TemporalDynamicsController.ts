...
export function useTemporalDynamicsController(
  ...
  const loadTemporalDynamics = useCallback(async (
    timeScale: TimeScale
    ...
    try {
      ...
      const result = await temporalService.getTemporalDynamics(patientId, timeScale);
      ...
      if (result.success && result.data) {
        ...
        setState(prevState => {
          ...
          const anomaliesDetected = patterns.filter(p => p.class === 'anomaly').length;
      ...
      setState(prevState => ({
        ...prevState,
        isProcessing: false,
        errorState: error instanceof Error ? error.message : 'Unknown error loading dynamics'
      }));
      
      return failure(error instanceof Error ? error.message : 'Unknown error loading dynamics');
    }
  }, [patientId]);
  ...
  const analyzePatterns = useCallback(async (): Promise<Result<TemporalPattern[]>> => {
    try {
      ...
      setState(prevState => ({
        ...prevState,
        isProcessing: false,
        errorState: error instanceof Error ? error.message : 'Unknown error analyzing patterns'
      }));
      
      return failure(error instanceof Error ? error.message : 'Unknown error analyzing patterns');
    }
  ...
  const detectTransitions = useCallback(async (): Promise<Result<StateTransition[]>> => {
    try {
      ...
      setState(prevState => ({
        ...prevState,
        isProcessing: false,
        errorState: error instanceof Error ? error.message : 'Unknown error detecting transitions'
      }));
      
      return failure(error instanceof Error ? error.message : 'Unknown error detecting transitions');
    }
  ...
  const extractFeatures = useCallback(async (
    ...
    try {
      ...
      setState(prevState => ({
        ...prevState,
        isProcessing: false,
        errorState: error instanceof Error ? error.message : 'Unknown error extracting features'
      }));
      
      return failure(error instanceof Error ? error.message : 'Unknown error extracting features');
    }
  ...
  const correlateWithClinicalEvents = useCallback(async (): Promise<Result<TemporalPattern[]>> => {
    try {
      ...
      return failure(result.error || 'Failed to correlate with clinical events');
    } catch (error) {
      return failure(error instanceof Error ? error.message : 'Unknown error correlating events');
    }
  }, [patientId, state.detectedPatterns, config.timeScales]);
  
  // Get early warning indicators
  const getEarlyWarningIndicators = useCallback(async (): Promise<Result<CriticalTransitionIndicator[]>> => {
    try {
      ...
      return failure(error instanceof Error ? error.message : 'Unknown error getting indicators');
    }
  ...
  const getPatternsForTimeScale = useCallback((timeScale: TimeScale): TemporalPattern[] => {
    return state.detectedPatterns.filter(pattern => pattern.timeScale === timeScale);
  }, [state.detectedPatterns]);
  
  // Get critical transitions for a specific time scale
  const getCriticalTransitionsForTimeScale = useCallback((timeScale: TimeScale): CriticalTransitionIndicator[] => {
    return state.criticalTransitions.filter(transition => transition.timeScale === timeScale);
