...
import { useBiometricStreamController } from './BiometricStreamController';
...
vi.mock('@application/services/biometricService', () => ({
  useBiometricService: () => ({
    fetchPatientStreams: vi.fn(() => Promise.resolve({
      success: true,
    ...
    activateStream: vi.fn(() => Promise.resolve({
      success: true,
      value: { status: 'active', message: 'Stream activated successfully' }
    })),
    deactivateStream: vi.fn(() => Promise.resolve({
      success: true,
      value: { status: 'paused', message: 'Stream paused successfully' }
    })),
    getLatestReadings: vi.fn(() => Promise.resolve({
      success: true,
      value: mockLatestReadings
    })),
    subscribeToAlerts: vi.fn(() => Promise.resolve({
      success: true,
    ...
    processRealtimeData: vi.fn(() => Promise.resolve({
      success: true, 
...
describe('BiometricStreamController', () => {
  ...
  it('initializes with correct default state and fetches stream configurations', async () => {
    ...
    expect(result.current.streamConfigurations).toEqual(mockStreamConfigurations);
  ...
  it('handles error states with neural precision', async () => {
    ...
    expect(result.current.isInitialized).toBe(true);
    expect(result.current.isLoading).toBe(false);
    expect(result.current.error).toBe('Failed to retrieve biometric stream configurations');
