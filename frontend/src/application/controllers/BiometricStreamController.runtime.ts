...
function isBiometricType(value: unknown): value is BiometricType {
    const validTypes: BiometricType[] = [
        ...
        "bloodGlucose", "cortisol", "sleepQuality", "eegThetaPower", "motionActivity"
    ];
...
function isAlertPriority(value: unknown): value is AlertPriority {
    const validPriorities: AlertPriority[] = ["informational", "warning", "urgent"];
    return typeof value === 'string' && validPriorities.includes(value as AlertPriority);
}
...
function isBiometricDataPoint(obj: unknown): obj is BiometricDataPoint {
  ...
  const isTimestampNumber = typeof dp.timestamp === 'number' && !isNaN(dp.timestamp);
  const isTimestampDateObject = dp.timestamp instanceof Date && !isNaN(dp.timestamp.getTime());
  const timestampValid = isTimestampNumber || isTimestampDateObject; // Removed broad string check
  const qualityValid = typeof dp.quality === 'string' && ['high', 'medium', 'low'].includes(dp.quality);
...
function isBiometricSource(value: unknown): value is BiometricSource {
    ...
    return typeof value === 'string' && validSources.includes(value as BiometricSource);
}
 ...
 */
export function validateBiometricData(data: unknown): Result<BiometricDataPoint, Error> {
 ...
 */
export function validatePartialStreamConfig(config: unknown): Result<PartialStreamConfig, Error> {
  if (typeof config !== 'object' || config === null) {
    return Err(new Error('Invalid Partial<StreamConfig>: Input must be an object.'));
  }
  const cfg = config as PartialStreamConfig;

  // Validate individual fields if they exist
  if (cfg.sampleRate !== undefined && (typeof cfg.sampleRate !== 'number' || cfg.sampleRate <= 0)) {
     return Err(new Error('Invalid Partial<StreamConfig>: sampleRate must be a positive number.'));
  }
  if (cfg.bufferSize !== undefined && (typeof cfg.bufferSize !== 'number' || cfg.bufferSize <= 0)) {
     return Err(new Error('Invalid Partial<StreamConfig>: bufferSize must be a positive number.'));
  }
  if (cfg.correlationWindow !== undefined && (typeof cfg.correlationWindow !== 'number' || cfg.correlationWindow <= 0)) {
     return Err(new Error('Invalid Partial<StreamConfig>: correlationWindow must be a positive number.'));
  }
  if (cfg.normalizeData !== undefined && typeof cfg.normalizeData !== 'boolean') {
     return Err(new Error('Invalid Partial<StreamConfig>: normalizeData must be a boolean.'));
  }
  if (cfg.filterOutliers !== undefined && typeof cfg.filterOutliers !== 'boolean') {
     return Err(new Error('Invalid Partial<StreamConfig>: filterOutliers must be a boolean.'));
  }
  if (cfg.streamIds !== undefined && (!Array.isArray(cfg.streamIds) || !cfg.streamIds.every(id => typeof id === 'string'))) {
     return Err(new Error('Invalid Partial<StreamConfig>: streamIds must be an array of strings.'));
  }
   if (cfg.sources !== undefined && (!Array.isArray(cfg.sources) || !cfg.sources.every(isBiometricSource))) {
     return Err(new Error('Invalid Partial<StreamConfig>: sources must be an array of valid BiometricSource values.'));
  }
  if (cfg.alertThresholds !== undefined) {
      if (!(cfg.alertThresholds instanceof Map)) {
          return Err(new Error('Invalid Partial<StreamConfig>: alertThresholds must be a Map.'));
      }
      for (const [key, value] of cfg.alertThresholds.entries()) {
          if (!isBiometricType(key)) {
              return Err(new Error(`Invalid Partial<StreamConfig>: Invalid key "${key}" in alertThresholds Map.`));
          }
          if (!Array.isArray(value) || !value.every(isBiometricThreshold)) {
               return Err(new Error(`Invalid Partial<StreamConfig>: Invalid threshold array for key "${key}" in alertThresholds Map.`));
          }
      }
  }
