...
export function useNeuralActivityController(patientId: string) {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        result.data.regionActivations.forEach(activation => {
          neuralState.metrics.activationLevels.set(activation.regionId, activation.level);
          
          // Set initially active regions
          if (activation.level === 'elevated' || activation.level === 'hyperactive') {
      ...
      return failure(error instanceof Error ? error.message : 'Unknown error loading baseline');
    }
  }, [patientId, neuralState]);
  
  // Apply neural transforms with mathematical precision
  const applyNeuralTransforms = useCallback((transforms: NeuralTransform[]): Result<void> => {
    try {
      ...
      sortedTransforms.forEach(transform => {
        ...
        const currentLevel = neuralState.metrics.activationLevels.get(transform.regionId) || 'baseline';
      ...
      return failure(error instanceof Error ? error.message : 'Unknown error applying transforms');
    }
  }, [neuralState]);
  ...
  const generateSymptomTransforms = useCallback(async (
    symptomIds: string[]
  ): Promise<Result<NeuralTransform[]>> => {
    try {
      const mappingsResult = await clinicalService.getSymptomMappings(patientId);
      
      if (!mappingsResult.success || !mappingsResult.data) {
        return failure(mappingsResult.error || 'Failed to load symptom mappings');
      }
      
      const relevantMappings = mappingsResult.data.filter(
        mapping => symptomIds.includes(mapping.symptomId)
      );
      ...
      return failure(error instanceof Error ? error.message : 'Unknown error generating transforms');
    }
  }, [patientId]);
  
  // Generate medication-driven neural transforms
  const generateMedicationTransforms = useCallback(async (
    medicationIds: string[]
  ): Promise<Result<NeuralTransform[]>> => {
    try {
      const medicationsResult = await clinicalService.getMedicationEffects(medicationIds);
      
      if (!medicationsResult.success || !medicationsResult.data) {
        return failure(medicationsResult.error || 'Failed to load medication effects');
      }
  ...
  const applySymptomActivity = useCallback(async (
    symptomIds: string[]
  ): Promise<Result<void>> => {
    try {
      ...
      if (!transformsResult.success || !transformsResult.data) {
        return failure(transformsResult.error || 'Failed to generate symptom transforms');
      }
      
      // Apply transforms
      return applyNeuralTransforms(transformsResult.data);
    } catch (error) {
      return failure(error instanceof Error ? error.message : 'Unknown error applying symptom activity');
    }
  }, [loadBaselineActivity, generateSymptomTransforms, applyNeuralTransforms]);
  
  // Apply medication-based activity changes
  const applyMedicationActivity = useCallback(async (
    medicationIds: string[]
  ): Promise<Result<void>> => {
    try {
      // Ensure baseline is loaded
      ...
      if (!transformsResult.success || !transformsResult.data) {
        return failure(transformsResult.error || 'Failed to generate medication transforms');
      }
      
      // Apply transforms
      return applyNeuralTransforms(transformsResult.data);
    } catch (error) {
      return failure(error instanceof Error ? error.message : 'Unknown error applying medication activity');
    }
  ...
  const resetToBaseline = useCallback(async (): Promise<Result<void>> => {
    try {
      ...
      return failure(error instanceof Error ? error.message : 'Unknown error resetting to baseline');
    }
  ...
  const setComputationalIntensity = useCallback((
    intensity: NeuralState['computationalIntensity']
  ): void => {
