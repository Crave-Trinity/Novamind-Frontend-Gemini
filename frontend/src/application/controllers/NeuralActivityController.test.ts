...
vi.mock('@application/services/brainService', () => ({
  useBrainService: () => ({
    fetchBrainModel: vi.fn(() => Promise.resolve({
      success: true,
      value: {
        id: 'brain-model-123',
    ...
    searchBrainModels: vi.fn(() => Promise.resolve({
      success: true,
      value: [
    ...
    fetchRegionDetails: vi.fn(() => Promise.resolve({
      success: true,
      value: mockRegionDetails
    })),
    analyzeConnectivity: vi.fn(() => Promise.resolve({
      success: true,
      value: mockConnectivityAnalysis
    })),
    getBaselineActivity: vi.fn(() => Promise.resolve({
      success: true,
      value: mockBaselineActivity
    })),
    generateActivityMap: vi.fn(() => Promise.resolve({
      success: true,
      value: 'generation-id-123'
    })),
    checkGenerationStatus: vi.fn(() => Promise.resolve({
      success: true,
...
vi.mock('@application/services/clinicalService', () => ({
  useClinicalService: () => ({
    fetchSymptomMappings: vi.fn(() => Promise.resolve({
      success: true,
      value: mockSymptomMappings
    })),
    fetchDiagnosisMappings: vi.fn(() => Promise.resolve({
      success: true,
      value: mockDiagnosisMappings
    })),
    getSymptomMappings: vi.fn(() => Promise.resolve({
      success: true,
      value: mockSymptomMappings
    })),
    getMedicationEffects: vi.fn(() => Promise.resolve({
      success: true,
...
const mockBrainRegions = [
  ...
  { id: 'hippocampus', name: 'Hippocampus', coordinates: { x: 15, y: 25, z: 35 } },
...
const mockRegionDetails = {
  ...
  clinicalSignificance: 'Implicated in anxiety disorders, PTSD, and emotional regulation',
  functionalGroups: ['limbic system', 'emotional processing'],
  connectionStrength: [
    { target: 'pfc', strength: 0.7, significance: 'Top-down emotional regulation' },
    { target: 'hippocampus', strength: 0.5, significance: 'Emotional memory formation' }
  ]
};
...
const mockBaselineActivity = {
  regions: {
    amygdala: { activation: 'high', significance: 'Elevated emotional reactivity' },
    pfc: { activation: 'moderate', significance: 'Normal executive function' },
    hippocampus: { activation: 'low', significance: 'Potential memory deficits' }
  },
  patterns: [
    { 
...
const mockMedicationEffects = [
  {
    ...
    neuralTargets: [
      { region: 'raphe-nuclei', expectedEffect: 'increased serotonergic transmission' },
      { region: 'pfc', expectedEffect: 'normalized activity', timeframe: '2-4 weeks' },
      { region: 'amygdala', expectedEffect: 'reduced reactivity', timeframe: '4-8 weeks' }
    ],
    functionalEffects: [
    ...
    biomarkerEffects: [
      { marker: 'amygdala reactivity', expectedChange: -0.3, timeframe: '4-8 weeks' },
      { marker: 'pfc regulation', expectedChange: 0.25, timeframe: '2-6 weeks' }
    ]
  }
];
...
describe('NeuralActivityController', () => {
  ...
  it('initializes with correct default state and queues model fetch', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useNeuralActivityController(mockPatientId));
  ...
  it('analyzes neural connectivity with clinical precision', async () => {
    ...
    // Check the resulting state
    expect(result.current.connectivityAnalysis).toEqual(mockConnectivityAnalysis);
    expect(result.current.error).toBeNull();
  });
  ...
  it('maps symptoms to neural activity with mathematical precision', async () => {
    ...
    const anxietyMapping = result.current.symptomMappings?.find(m => m.symptomId === 'anxiety');
