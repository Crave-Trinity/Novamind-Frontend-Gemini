...
function isAggregationMethod(value: unknown): value is AggregationMethod {
  ...
  return (
    typeof value === "string" &&
    validMethods.includes(value as AggregationMethod)
  );
}
...
export function validateSymptomPredictionParams(
  ...
  if (typeof params !== "object" || params === null) {
    return Err(
      new Error("Invalid SymptomPredictionParams: Input must be an object."),
    );
  }
  ...
  if (typeof p.patientId !== "string")
    return Err(
      new Error("Invalid SymptomPredictionParams: patientId must be a string."),
    );
  if (!isStringArray(p.symptomIds))
    return Err(
      new Error(
        "Invalid SymptomPredictionParams: symptomIds must be an array of strings.",
      ),
    );
  if (typeof p.horizon !== "number" || p.horizon <= 0)
    return Err(
      new Error(
        "Invalid SymptomPredictionParams: horizon must be a positive number.",
      ),
    );
  if (typeof p.includeBiomarkers !== "boolean")
    return Err(
      new Error(
        "Invalid SymptomPredictionParams: includeBiomarkers must be a boolean.",
      ),
    );
  if (typeof p.includeEnvironmentalFactors !== "boolean")
    return Err(
      new Error(
        "Invalid SymptomPredictionParams: includeEnvironmentalFactors must be a boolean.",
      ),
    );
  if (!Array.isArray(p.models))
    return Err(
      new Error("Invalid SymptomPredictionParams: models must be an array."),
    ); // Basic check as PredictionModel is 'any'
  if (!isAggregationMethod(p.aggregationMethod))
    return Err(
      new Error("Invalid SymptomPredictionParams: invalid aggregationMethod."),
    );
...
export function validateTreatmentPredictionParams(
  ...
  if (typeof params !== "object" || params === null) {
    return Err(
      new Error("Invalid TreatmentPredictionParams: Input must be an object."),
    );
  }
  ...
  if (typeof p.patientId !== "string")
    return Err(
      new Error(
        "Invalid TreatmentPredictionParams: patientId must be a string.",
      ),
    );
  if (!isStringArray(p.treatmentIds))
    return Err(
      new Error(
        "Invalid TreatmentPredictionParams: treatmentIds must be an array of strings.",
      ),
    );
  if (typeof p.horizon !== "number" || p.horizon <= 0)
    return Err(
      new Error(
        "Invalid TreatmentPredictionParams: horizon must be a positive number.",
      ),
    );
  if (typeof p.includeBiomarkers !== "boolean")
    return Err(
      new Error(
        "Invalid TreatmentPredictionParams: includeBiomarkers must be a boolean.",
      ),
    );
  if (typeof p.includeEnvironmentalFactors !== "boolean")
    return Err(
      new Error(
        "Invalid TreatmentPredictionParams: includeEnvironmentalFactors must be a boolean.",
      ),
    );
  if (!Array.isArray(p.models))
    return Err(
      new Error("Invalid TreatmentPredictionParams: models must be an array."),
    );
  if (!isAggregationMethod(p.aggregationMethod))
    return Err(
      new Error(
        "Invalid TreatmentPredictionParams: invalid aggregationMethod.",
      ),
    );
...
export function validateRelapsePredictionParams(
  ...
  if (typeof params !== "object" || params === null) {
    return Err(
      new Error("Invalid RelapsePredictionParams: Input must be an object."),
    );
  }
  ...
  if (typeof p.patientId !== "string")
    return Err(
      new Error("Invalid RelapsePredictionParams: patientId must be a string."),
    );
  if (!isStringArray(p.disorderIds))
    return Err(
      new Error(
        "Invalid RelapsePredictionParams: disorderIds must be an array of strings.",
      ),
    );
  if (typeof p.horizon !== "number" || p.horizon <= 0)
    return Err(
      new Error(
        "Invalid RelapsePredictionParams: horizon must be a positive number.",
      ),
    );
  if (typeof p.includeBiomarkers !== "boolean")
    return Err(
      new Error(
        "Invalid RelapsePredictionParams: includeBiomarkers must be a boolean.",
      ),
    );
  if (typeof p.includeEnvironmentalFactors !== "boolean")
    return Err(
      new Error(
        "Invalid RelapsePredictionParams: includeEnvironmentalFactors must be a boolean.",
      ),
    );
  if (!Array.isArray(p.models))
    return Err(
      new Error("Invalid RelapsePredictionParams: models must be an array."),
    );
  if (!isAggregationMethod(p.aggregationMethod))
    return Err(
      new Error("Invalid RelapsePredictionParams: invalid aggregationMethod."),
    );
...
export function validateRiskAssessmentParams(
  params: unknown,
): Result<RiskAssessmentParams, Error> {
  if (typeof params !== "object" || params === null) {
    return Err(
      new Error("Invalid RiskAssessmentParams: Input must be an object."),
    );
  }
  ...
  if (typeof p.patientId !== "string")
    return Err(
      new Error("Invalid RiskAssessmentParams: patientId must be a string."),
    );
  if (!isStringArray(p.riskFactors))
    return Err(
      new Error(
        "Invalid RiskAssessmentParams: riskFactors must be an array of strings.",
      ),
    );
  if (typeof p.includeBiomarkers !== "boolean")
    return Err(
      new Error(
        "Invalid RiskAssessmentParams: includeBiomarkers must be a boolean.",
      ),
    );
  if (typeof p.includeEnvironmentalFactors !== "boolean")
    return Err(
      new Error(
        "Invalid RiskAssessmentParams: includeEnvironmentalFactors must be a boolean.",
      ),
    );
  if (!Array.isArray(p.models))
    return Err(
      new Error("Invalid RiskAssessmentParams: models must be an array."),
    );
  if (!isAggregationMethod(p.aggregationMethod))
    return Err(
      new Error("Invalid RiskAssessmentParams: invalid aggregationMethod."),
    );
...
export function validateConfigurePredictionParams(
  ...
  if (typeof params !== "object" || params === null) {
    return Err(
      new Error("Invalid ConfigurePredictionParams: Input must be an object."),
    );
  }
  ...
  if (
    ...
    return Err(
      new Error(
        "Invalid ConfigurePredictionParams: predictionHorizon must be a positive number.",
      ),
    );
  if (p.activeModels !== undefined && !Array.isArray(p.activeModels))
    return Err(
      new Error(
        "Invalid ConfigurePredictionParams: activeModels must be an array.",
      ),
    );
  if (
    p.aggregationMethod !== undefined &&
    !isAggregationMethod(p.aggregationMethod)
  )
    return Err(
      new Error(
        "Invalid ConfigurePredictionParams: invalid aggregationMethod.",
      ),
    );
  if (
    p.includeBiomarkers !== undefined &&
    typeof p.includeBiomarkers !== "boolean"
  )
    return Err(
      new Error(
        "Invalid ConfigurePredictionParams: includeBiomarkers must be a boolean.",
      ),
    );
  if (
    ...
    return Err(
      new Error(
        "Invalid ConfigurePredictionParams: includeEnvironmentalFactors must be a boolean.",
      ),
    );
