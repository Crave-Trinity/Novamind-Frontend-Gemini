...
const NeuralConnections: React.FC<NeuralConnectionsProps> = ({
  ...
  const getRegionPosition = useCallback((regionId: string): [number, number, number] => {
  ...
  const filteredConnections = useMemo(() => {
    ...
    if (filterByActivity && renderMode === RenderMode.FUNCTIONAL) {
      filtered = filtered.filter(conn => {
        ...
        return (sourceRegion && sourceRegion.activityLevel > 0.3) || 
  ...
  const isConnectionActive = useCallback((conn: NeuralConnection): boolean => {
    // Connection is active if either connected region is selected
    return safeSelectedIds.includes(conn.sourceId) || 
           safeSelectedIds.includes(conn.targetId);
  }, [safeSelectedIds]);
  
  const isConnectionHighlighted = useCallback((conn: NeuralConnection): boolean => {
    // Connection is highlighted if either connected region is highlighted
    return safeHighlightedIds.includes(conn.sourceId) || 
           safeHighlightedIds.includes(conn.targetId);
  }, [safeHighlightedIds]);
  
  // Calculate connection activity level based on connected regions
  const getConnectionActivity = useCallback((conn: NeuralConnection): number => {
    const sourceRegion = regionsById.get(conn.sourceId);
    const targetRegion = regionsById.get(conn.targetId);
  ...
  const getConnectionColor = useCallback((conn: NeuralConnection): string => {
      ...
      {filteredConnections.map(conn => {
        ...
        return (
          <ConnectionLine
            ...
            flowDirection={conn.type === 'bidirectional' ? 'bidirectional' : 'forward'}
