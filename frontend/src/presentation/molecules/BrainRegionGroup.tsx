...
const BrainRegionGroup: React.FC<BrainRegionGroupProps> = ({
  ...
  const getRegionColor = useCallback((region: BrainRegion): string => {
  ...
  const getRegionSize = useCallback((region: BrainRegion): number => {
  ...
  if (useInstancing && instancedData) {
    ...
    return (
      <group position={position} rotation={rotation as any}>
        ...
        {/* Optional labels */}
        {showLabels && filteredRegions.map(region => {
          const [x, y, z] = Array.isArray(region.position)
            ? region.position
            : [region.position.x, region.position.y, region.position.z];
