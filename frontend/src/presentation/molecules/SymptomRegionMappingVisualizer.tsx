    ...
    mapping.activationPatterns.forEach(pattern => {
      pattern.regionActivations.forEach(activation => {
        ...
        if (region) {
          ...
          const midPoint = new Vector3().addVectors(symptomPosition, region.position).multiplyScalar(0.5);
          const controlPoint = new Vector3().copy(midPoint).add(new Vector3(
            (Math.random() - 0.5) * 3,
    ...
    mapping.activationPatterns.forEach(pattern => {
      pattern.regionActivations.forEach(activation => {
        ...
        if (region) {
          ...
          // Calculate control point for curved connections
          const midPoint = new Vector3().addVectors(diagnosisPosition, region.position).multiplyScalar(0.5);
          const controlPoint = new Vector3().copy(midPoint).add(new Vector3(
            (Math.random() - 0.5) * 3,
 ...
 */
function createCurvePoints(start: Vector3, end: Vector3, control: Vector3, segments: number = 20): Vector3[] {
...
export const SymptomRegionMappingVisualizer: React.FC<SymptomRegionMappingVisualizerProps> = ({
  ...
  const visibleConnections = useMemo(() => {
    ...
    filteredConnections.sort((a, b) => {
      ...
      const aActive = activeSymptoms.some(s => s.id === a.symptomId) || 
                     activeDiagnoses.some(d => d.id === a.symptomId);
      const bActive = activeSymptoms.some(s => s.id === b.symptomId) ||
  ...
  return (
    <group>
      ...
      {visibleConnections.map(conn => {
        ...
        // Animation settings
        const dashArray = enableAnimation ? [0.1, conn.isPrimary ? 0.1 : 0.15] : undefined;
      ...
      {showSymptomLabels && symptomGroups.map(group => {
        const isSelected = group.connections[0]?.symptomId === selectedSymptomId || 
          ...
          >
            <div
              style={{
                ...
                borderRadius: '1rem',
                backgroundColor: `${primaryConn?.color}${Math.round(opacity * 255).toString(16).padStart(2, '0')}`,
                color: 'white',
                fontSize: '0.875rem',
                fontWeight: 500,
                whiteSpace: 'nowrap',
              ...
              onClick={() => handleSymptomClick(group.connections[0]?.symptomId || '')}
            >
              {group.isDiagnosis ? '🏥 ' : '🔍 '}
