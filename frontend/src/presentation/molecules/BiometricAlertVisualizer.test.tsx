...
import BiometricAlertVisualizer, { ClinicalAlert } from './BiometricAlertVisualizer';
import { Vector3 } from 'three';
import { BrainRegion } from '@domain/types/brain/models';

// Mock Three.js and related components to prevent WebGL errors
vi.mock('@react-three/fiber', () => ({
  Html: ({ children }: { children: React.ReactNode }) => <div data-testid="alert-html-overlay">{children}</div>,
  useFrame: vi.fn((callback) => callback({ camera: { position: { x: 0, y: 0, z: 10 } } }, 0)),
}));

vi.mock('@react-three/drei', () => ({
  Text: ({ children, color }: any) => (
    <div data-testid="alert-text" data-color={color}>{children}</div>
  ),
...
const mockAlerts: ClinicalAlert[] = [
  {
    ...
    description: 'Heart rate exceeds 100 BPM for 10+ minutes during rest period',
  ...
  },
  {
    ...
    description: 'Social interaction decreased by 40% compared to 7-day average',
...
describe('BiometricAlertVisualizer', () => {
  ...
  it('filters alerts by priority with clinical precision', () => {
    ...
    expect(screen.getByText(/elevated heart rate/i)).toBeInTheDocument();
    expect(screen.queryByText(/reduced social interaction/i)).not.toBeInTheDocument();
  ...
  it('applies different positioning modes correctly', () => {
    ...
    expect(screen.getAllByTestId('alert-html-overlay').length).toBeGreaterThan(0);
  });
