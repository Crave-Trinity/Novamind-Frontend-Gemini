...
const BrainRegionLabels: React.FC<BrainRegionLabelsProps> = ({
  ...
  const regionsToLabel = useMemo(() => {
    ...
    if (!showAllLabels) {
      ...
      const otherRegions = sorted.filter(r => !priorityIds.has(r.id)).slice(0, maxLabels - priorityRegions.size());
        ...
        return (
          <Html
            ...
            calculatePosition={(el, cam, size) => {
              ...
              const scale = Math.max(0.6, Math.min(1, 10 / distance)) * labelScale;
              ...
              {(clinical.indicators.length > 0 && (isSelected || isHighlighted)) && (
                ...
                `}>
                  {clinical.indicators.map((indicator, i) => (
                    <div key={i} className="whitespace-normal">• {indicator}</div>
                  ))}
                </div>
              )}
            </div>
