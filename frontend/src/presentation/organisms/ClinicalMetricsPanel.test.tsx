...
vi.mock('@presentation/atoms/ScrollArea', () => ({
  ...
  ScrollBar: ({ orientation }: any) => <div data-testid={`scroll-bar-${orientation}`}></div>,
}));
...
vi.mock('lucide-react', () => ({
  ...
  ChevronRight: () => <div data-testid="icon-chevron-right">Chevron Right Icon</div>,
...
const mockMetrics = {
  ...
  brainActivity: {
    regions: [
      { name: 'Amygdala', activation: 'high', clinicalSignificance: 'anxiety correlation' },
      { name: 'Prefrontal Cortex', activation: 'reduced', clinicalSignificance: 'executive function deficit' },
      { name: 'Hippocampus', activation: 'normal', clinicalSignificance: 'memory intact' }
    ],
...
describe('ClinicalMetricsPanel', () => {
  ...
  it('handles error states with clinical precision', () => {
    ...
    expect(screen.getByText(/Failed to load clinical metrics/i)).toBeInTheDocument();
  });
  ...
  it('responds to timeframe changes with quantum precision', async () => {
    ...
    const timeframeButton = screen.getByText(/7 Days/i) || screen.getByText(/30 Days/i) || screen.getByText(/90 Days/i);
