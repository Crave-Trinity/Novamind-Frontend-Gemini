...
import { CriticalTransitionIndicator, TemporalPattern } from '@domain/types/temporal/dynamics';
...
export const ClinicalMetricsPanel: React.FC<ClinicalMetricsPanelProps> = ({
  ...
  const activationMetrics = useMemo(() => {
    ...
    const topActiveRegions = Array.from(state.neuralActivation.entries())
      ...
      .sort((a, b) => (a.level === 'hyperactive' && b.level !== 'hyperactive') ? -1 : 1)
      .slice(0, 5);
  ...
  const temporalMetrics = useMemo(() => {
    ...
    const stabilityIndex = Math.max(0, 100 - (criticalTransitions.length * 20) - (patternCounts.anomaly * 10));
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="bg-slate-700/50 rounded-md p-2 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-slate-400">Elevated Activity</span>
                        <Badge 
                          variant="outline" 
                    ...
                    <div className="bg-slate-700/50 rounded-md p-2 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs text-slate-400">Hyperactive</span>
                        <Badge 
                          variant="outline" 
                    ...
                    {activationMetrics.topActiveRegions.length > 0 ? (
                      <div className="space-y-2">
                        {activationMetrics.topActiveRegions.map(region => (
                          ...
                          >
                            <span className="text-xs text-white truncate max-w-[70%]">{region.name}</span>
                            <Badge 
                              variant="outline" 
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-700/50 rounded-md p-3">
                    {temporalMetrics.criticalTransitions.length > 0 ? (
                      <div className="space-y-2">
                        {temporalMetrics.criticalTransitions.map((transition, index) => (
                          <div 
                            key={index} 
                            ...
                            <div className="flex items-center">
                              ...
                              <div>
                                ...
                                <div className="text-xs text-slate-400">
                                  ...
                                  {transition.description && transition.description.length > 30 ? '...' : ''}
                                </div>
                              </div>
                            </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-slate-300">Neural Stability</h3>
                    <span className="text-xs text-slate-400">
