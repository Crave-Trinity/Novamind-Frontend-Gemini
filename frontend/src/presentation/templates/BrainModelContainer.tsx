...
interface BrainModelContainerProps {
  patientId: string;
  initialRenderMode?: 'standard' | 'heatmap' | 'connectivity' | 'activity' | 'treatment';
...
const VisualizationCanvas: React.FC<{
  ...
  const visualizationOptions = useMemo<VisualizationOptions>(() => ({
    ...
    showConnections: renderMode === 'connectivity' || renderMode === 'standard',
  ...
  const connectionArray = useMemo(() => {
    ...
    return brainModel.connections.map(connection => ({
      id: connection.id,
      strength: connectionStrengths.get(`${connection.sourceId}-${connection.targetId}`) || connection.strength
    }));
  ...
  const regionLookup = useMemo(() => {
    if (!brainModel || !brainModel.regions) return new Map<string, BrainRegion>();
...
const SceneSetup: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    camera.position.set(CAMERA_SETTINGS.position[0], CAMERA_SETTINGS.position[1], CAMERA_SETTINGS.position[2]);
