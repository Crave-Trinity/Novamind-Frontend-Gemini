...
export const NeuralCorrelationBadge: React.FC<NeuralCorrelationBadgeProps> = ({
  ...
  const correlationIcon = useMemo(() => {
    if (correlation.strength >= 0.8) {
      ...
      return <Brain className={size === 'sm' ? 'h-3 w-3' : size === 'lg' ? 'h-5 w-5' : 'h-4 w-4'} />;
    } else {
      return <Activity className={size === 'sm' ? 'h-3 w-3' : size === 'lg' ? 'h-5 w-5' : 'h-4 w-4'} />;
    }
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1 mt-0.5">
              {correlation.regions.map((region, i) => (
                <span key={i} className="text-xs bg-indigo-900/60 text-indigo-100 px-1.5 py-0.5 rounded">
                  {region}
                </span>
              ))}
            </div>
