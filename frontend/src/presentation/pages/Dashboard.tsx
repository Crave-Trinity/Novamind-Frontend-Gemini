...
const Dashboard: React.FC = () => {
  ...
  if (isPatientsLoading) {
    return (
      <div className="flex justify-center items-center h-screen bg-background dark:bg-background p-6">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-primary-500 mb-4"></div>
          <h3 className="text-xl font-medium text-neutral-800 dark:text-white">Loading Dashboard</h3>
          <p className="text-neutral-600 dark:text-neutral-400 mt-2">Retrieving patient data...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex justify-center items-center h-screen bg-background dark:bg-background p-6">
        <div className="text-center max-w-lg">
          <div className="rounded-full h-12 w-12 bg-red-100 text-red-600 dark:bg-red-900 dark:text-red-300 flex items-center justify-center mb-4 mx-auto">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <h3 className="text-xl font-medium text-neutral-800 dark:text-white">Error Loading Dashboard</h3>
          <p className="text-neutral-600 dark:text-neutral-400 mt-2">
            {patientsError instanceof Error ? patientsError.message : 'Failed to load patient data. Please try again.'}
          </p>
          <button
  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-background p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          ...
          <div className="mt-4 sm:mt-0 flex items-center gap-4">
            {/* Theme Toggle */}
            <button
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white dark:bg-background-card text-neutral-700 dark:text-neutral-300 text-sm shadow-sm"
        ...
        {/* Patient Selection */}
        <div className="bg-white dark:bg-background-card rounded-xl shadow-sm mb-6 p-4">
        ...
        {selectedPatientId && digitalTwinProfile ? (
          ...
          <div className="bg-white dark:bg-background-card rounded-xl shadow-sm p-8 text-center">
            <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-primary-500 mb-4 mx-auto"></div>
            <h3 className="text-lg font-medium text-neutral-800 dark:text-white">Loading Digital Twin</h3>
            <p className="text-neutral-600 dark:text-neutral-400 mt-2">Retrieving profile data...</p>
          </div>
        ) : profileError ? (
          <div className="bg-white dark:bg-background-card rounded-xl shadow-sm p-8 text-center">
            <div className="rounded-full h-10 w-10 bg-red-100 text-red-600 dark:bg-red-900 dark:text-red-300 flex items-center justify-center mb-4 mx-auto">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <h3 className="text-lg font-medium text-neutral-800 dark:text-white">Error Loading Digital Twin</h3>
            <p className="text-neutral-600 dark:text-neutral-400 mt-2">
              {profileError instanceof Error ? profileError.message : 'Failed to load digital twin profile. Please try again.'}
            </p>
            <Button
              variant="primary"
              size="sm"
