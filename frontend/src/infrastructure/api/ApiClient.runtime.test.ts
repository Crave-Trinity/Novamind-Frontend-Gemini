...
describe('ApiClient Runtime Validation', () => {
  ...
  describe('validateApiResponse', () => {
    ...
    it('should return Err for data not matching the guard (missing field)', () => {
      const result = validateApiResponse(mockInvalidPatient_MissingName, isApiPatient, 'Patient Data');
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('Invalid Patient Data');
      expect((result.val as Error).message).toContain('does not match expected structure');
    });

     it('should return Err for data not matching the guard (wrong type)', () => {
      const result = validateApiResponse(mockInvalidPatient_WrongIdType, isApiPatient, 'Patient Data');
      expect(result.err).toBe(true);
      expect((result.val as Error).message).toContain('Invalid Patient Data');
    });

     it('should return Err for array data not matching the array guard', () => {
      const result = validateApiResponse(mockInvalidPatientArray, isApiPatientArray, 'Patient List');
      expect(result.err).toBe(true);
