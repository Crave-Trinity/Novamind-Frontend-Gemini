...
export function isPatientModel(value: unknown): value is PatientModel {
  ...
  return (
    ...
    patient.contactInformation && typeof patient.contactInformation === 'object' &&
    patient.demographics && isPatientDemographics(patient.demographics) &&
    patient.clinicalHistory && isClinicalHistory(patient.clinicalHistory) &&
    Array.isArray(patient.medications) && patient.medications.every(isMedication) &&
    Array.isArray(patient.symptoms) && patient.symptoms.every(isSymptom) &&
    Array.isArray(patient.treatmentResponses) && patient.treatmentResponses.every(isTreatmentResponse) &&
