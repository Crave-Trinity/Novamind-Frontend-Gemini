...
export const PatientModelValidator = {
  ...
  isValid: (obj: unknown): obj is PatientModel => {
    ...
    return (
      ...
      patient.demographics && PatientDemographicsValidator.isValid(patient.demographics) &&
      patient.clinicalHistory && ClinicalHistoryValidator.isValid(patient.clinicalHistory) &&
      Array.isArray(patient.medications) && patient.medications.every(MedicationValidator.isValid) &&
      Array.isArray(patient.symptoms) && patient.symptoms.every(SymptomValidator.isValid) &&
