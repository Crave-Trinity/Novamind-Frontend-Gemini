...
export function calculateNeuralActivation(
  ...
  try {
    ...
    safeActiveSymptoms.forEach(symptom => {
      ...
      const mapping = safeSymptomMappings.find(m => 
        m.symptomId === symptom.id || m.symptomName === symptom.name
      );
    ...
    safeActiveDiagnoses.forEach(diagnosis => {
      // Find relevant mapping
      const mapping = safeDiagnosisMappings.find(m => 
...
export function calculateTreatmentImpact(
  ...
  try {
    ...
    safeTreatmentIds.forEach(treatmentId => {
      ...
      new SafeArray(mapping.effectPatterns.normalizedConnectivity).forEach(([sourceId, targetId]) => {
        addOrUpdateConnectionImpact(impact.connectionImpacts, sourceId, targetId, 'normalize', 0.6, 0.7);
      });
    ...
    if (impact.regionImpacts.length > 0) {
      const avgMagnitude = impact.regionImpacts.reduce((sum, ri) => sum + ri.magnitude, 0) / 
                           impact.regionImpacts.length;
      
      impact.overallSeverity = 
        avgMagnitude > 0.8 ? RiskLevel.HIGH :
        avgMagnitude > 0.5 ? RiskLevel.MODERATE :
        avgMagnitude > 0.2 ? RiskLevel.LOW :
        RiskLevel.NONE;
    }
