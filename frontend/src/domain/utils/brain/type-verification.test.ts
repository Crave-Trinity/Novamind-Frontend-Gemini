...
describe("Brain type verification", () => {
  describe("verifyVector3", () => {
    ...
    it("fails on non-object values", () => {
      expect(brainTypeVerifier.verifyVector3("not an object").success).toBe(
        false,
      );
    ...
    it("fails when coordinates are not numbers", () => {
      expect(
        brainTypeVerifier.verifyVector3({ x: "1", y: 2, z: 3 }).success,
      ).toBe(false);
      expect(
        brainTypeVerifier.verifyVector3({ x: 1, y: true, z: 3 }).success,
      ).toBe(false);
      expect(
        brainTypeVerifier.verifyVector3({ x: 1, y: 2, z: null }).success,
      ).toBe(false);
    });

    it("fails when coordinates are missing", () => {
      expect(brainTypeVerifier.verifyVector3({ x: 1, y: 2 }).success).toBe(
        false,
      );
      expect(brainTypeVerifier.verifyVector3({ y: 2, z: 3 }).success).toBe(
        false,
      );
  ...
  describe("safelyParseVector3", () => {
    it("returns Vector3 for valid objects", () => {
      expect(
        brainTypeVerifier.safelyParseVector3({ x: 1, y: 2, z: 3 }),
      ).toEqual({ x: 1, y: 2, z: 3 });
    });
  ...
  describe("verifyRenderMode", () => {
    ...
    it("fails on invalid RenderMode values", () => {
      expect(brainTypeVerifier.verifyRenderMode("INVALID_MODE").success).toBe(
        false,
      );
  ...
  describe("verifyBrainRegion", () => {
    ...
    it("fails when required properties are missing", () => {
      ...
      expect(brainTypeVerifier.verifyBrainRegion(missingProps).success).toBe(
        false,
      );
    });
    ...
    it("fails when properties have wrong types", () => {
      ...
      expect(brainTypeVerifier.verifyBrainRegion(wrongTypes).success).toBe(
        false,
      );
    });
  });
  ...
  describe("verifyNeuralConnection", () => {
    ...
    it("accepts optional properties", () => {
      ...
      const result = brainTypeVerifier.verifyNeuralConnection(
        connectionWithOptionals,
      );
      expect(result.success).toBe(true);
    ...
    it("fails when required properties are missing", () => {
      ...
      expect(
        brainTypeVerifier.verifyNeuralConnection(missingProps).success,
      ).toBe(false);
    });
  });
  ...
  describe("verifyBrainModel", () => {
    ...
    it("fails when required properties are missing", () => {
      ...
      expect(brainTypeVerifier.verifyBrainModel(missingProps).success).toBe(
        false,
      );
    });
    ...
    it("fails when arrays contain invalid items", () => {
      ...
      expect(brainTypeVerifier.verifyBrainModel(invalidItems).success).toBe(
        false,
      );
    });
  });
  ...
  describe("assertion functions", () => {
    it("assertVector3 passes for valid Vector3", () => {
      expect(() =>
        brainTypeVerifier.assertVector3({ x: 1, y: 2, z: 3 }),
      ).not.toThrow();
    });
    ...
    it("assertRenderMode passes for valid RenderMode", () => {
      expect(() =>
        brainTypeVerifier.assertRenderMode(RenderMode.ANATOMICAL),
      ).not.toThrow();
    });
    ...
    it("assertBrainRegion passes for valid BrainRegion", () => {
      ...
      expect(() =>
        brainTypeVerifier.assertBrainRegion(validRegion),
      ).not.toThrow();
    });
