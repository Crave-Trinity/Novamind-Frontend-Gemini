...
describe('Brain type verification', () => {
  ...
  describe('safelyParseVector3', () => {
    it('returns Vector3 for valid objects', () => {
      expect(brainTypeVerifier.safelyParseVector3({ x: 1, y: 2, z: 3 }))
        .toEqual({ x: 1, y: 2, z: 3 });
    });
    ...
    it('uses fallback for missing values', () => {
      expect(brainTypeVerifier.safelyParseVector3({ x: 1 }))
        .toEqual({ x: 1, y: 0, z: 0 });
    });
    
    it('returns fallback for non-object values', () => {
      expect(brainTypeVerifier.safelyParseVector3(null))
        .toEqual({ x: 0, y: 0, z: 0 });
      
      expect(brainTypeVerifier.safelyParseVector3('not an object', { x: 10, y: 20, z: 30 }))
  ...
  describe('assertion functions', () => {
    ...
    it('assertVector3 throws for invalid Vector3', () => {
      expect(() => brainTypeVerifier.assertVector3({ x: 1, y: '2', z: 3 }))
    ...
    it('assertRenderMode throws for invalid RenderMode', () => {
      expect(() => brainTypeVerifier.assertRenderMode('INVALID_MODE'))
        .toThrow(TypeVerificationError);
    });
    ...
    it('assertBrainRegion throws for invalid BrainRegion', () => {
      ...
      expect(() => brainTypeVerifier.assertBrainRegion(invalidRegion))
        .toThrow(TypeVerificationError);
    });
  });
});
