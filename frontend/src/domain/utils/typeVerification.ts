...
export class TypeVerificationError extends NeuralError {
  constructor(
    ...
    super(
      `Type Verification Error: ${message} (expected: ${expectedType}, received: ${receivedType}${field ? `, field: ${field}` : ""})`,
    );
...
export function verifyBrainModel(obj: unknown): Result<BrainModel> {
  ...
  for (const connection of model.connections) {
    const sourceExists = model.regions.some(
      (r) => r.id === connection.sourceId,
    );
    if (!sourceExists) {
      return {
        success: false,
    ...
    const targetExists = model.regions.some(
      (r) => r.id === connection.targetId,
    );
    if (!targetExists) {
      return {
        success: false,
...
export function verifyNeuralConnection(obj: unknown): Result<NeuralConnection> {
  ...
  // Optional properties
  if (
    connection.activityLevel !== undefined &&
 ...
 */
export function safelyParseBoolean(
  value: unknown,
 ...
 */
export function safelyParseString(
  value: unknown,
  fallback: string = "",
): string {
 ...
 */
export function safelyParseVector3(
  value: unknown,
 ...
 * @throws TypeVerificationError if the assertion fails
 */
export function assertBrainRegion(
  value: unknown,
 ...
 * @throws TypeVerificationError if the assertion fails
 */
export function assertNeuralConnection(
  value: unknown,
