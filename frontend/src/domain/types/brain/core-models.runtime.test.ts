...
describe("Brain Core Models runtime validators", () => {
  ...
  describe("CoordinateValidator", () => {
    it("validates valid Coordinate objects", () => {
      expect(
        CoordinateValidator.isValid({ x: 1, y: 2, z: 3, label: "Point A" }),
      ).toBe(true);
      expect(
        CoordinateValidator.isValid({ x: -1.5, y: 0, z: 3.14, label: "" }),
      ).toBe(true);
    });
    ...
    it("rejects invalid Coordinate objects", () => {
      ...
      expect(CoordinateValidator.isValid({ x: 1, y: 2, z: 3, label: 5 })).toBe(
        false,
      );
