...
export const NeuralTransformValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransform => {
    ...
    if (transform.frequencyBand !== undefined && typeof transform.frequencyBand !== 'string') return false;
    if (transform.clinicalCorrelationId !== undefined && typeof transform.clinicalCorrelationId !== 'string') return false;
    ...
    if (transform.confidence !== undefined && typeof transform.confidence !== 'number') return false;
    if (transform.clinicalNotes !== undefined && typeof transform.clinicalNotes !== 'string') return false;
    
    // Value range validations
    if (transform.activationChange < -1.0 || transform.activationChange > 1.0) return false;
    if (transform.confidence !== undefined && (transform.confidence < 0.0 || transform.confidence > 1.0)) return false;
...
export const NeuralTransformBatchValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransformBatch => {
    ...
    // Validate optional fields if present
    if (batch.clinicalContext !== undefined && typeof batch.clinicalContext !== 'string') return false;
...
export const NeuralTransformSequenceValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransformSequence => {
    ...
    // Validate optional fields if present
    if (sequence.repetitions !== undefined && 
        (typeof sequence.repetitions !== 'number' || sequence.repetitions < 0)) return false;
    
    if (sequence.interRepetitionDelayMs !== undefined && 
        (typeof sequence.interRepetitionDelayMs !== 'number' || sequence.interRepetitionDelayMs < 0)) return false;
    
    if (sequence.description !== undefined && typeof sequence.description !== 'string') return false;
    
    if (sequence.tags !== undefined && !Array.isArray(sequence.tags)) return false;
    if (sequence.tags !== undefined && !sequence.tags.every(tag => typeof tag === 'string')) return false;
...
export const NeuralTransformResultValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransformResult => {
    ...
    if (result.performanceMetrics !== undefined) {
      if (typeof result.performanceMetrics !== 'object' || result.performanceMetrics === null) return false;
      ...
      if (!['low', 'medium', 'high'].includes(result.performanceMetrics.computationalIntensity)) return false;
    }
