...
export const NeuralTransformValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransform => {
    ...
    if (!validTransitionTypes.includes(transform.transitionType as any))
      return false;
    if (typeof transform.sourceTrigger !== "string") return false;
    if (!validSourceTriggers.includes(transform.sourceTrigger as any))
      return false;

    // Validate optional fields if present
    if (transform.id !== undefined && typeof transform.id !== "string")
      return false;
    if (
      transform.frequencyBand !== undefined &&
      typeof transform.frequencyBand !== "string"
    )
      return false;
    if (
      ...
      return false;
    if (
      transform.timestamp !== undefined &&
      !(transform.timestamp instanceof Date)
    )
      return false;
    if (
      transform.duration !== undefined &&
      typeof transform.duration !== "number"
    )
      return false;
    if (
      ...
      return false;
    if (
      transform.clinicalNotes !== undefined &&
      typeof transform.clinicalNotes !== "string"
    )
      return false;
...
export const NeuralTransformBatchValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransformBatch => {
    ...
    // Validate optional fields if present
    if (
      batch.clinicalContext !== undefined &&
      typeof batch.clinicalContext !== "string"
    )
      return false;
...
export const NeuralTransformSequenceValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransformSequence => {
    ...
    if (
      ...
      (typeof sequence.interRepetitionDelayMs !== "number" ||
        sequence.interRepetitionDelayMs < 0)
    )
      return false;
    ...
    if (sequence.tags !== undefined && !Array.isArray(sequence.tags))
      return false;
    if (
      sequence.tags !== undefined &&
      !sequence.tags.every((tag) => typeof tag === "string")
    )
      return false;
...
export const NeuralTransformResultValidator = {
  ...
  isValid: (obj: unknown): obj is NeuralTransformResult => {
    ...
    if (!result.affectedRegions.every((region) => typeof region === "string"))
      return false;
    ...
    if (result.performanceMetrics !== undefined) {
      if (
        ...
        result.performanceMetrics === null
      )
        return false;
      if (typeof result.performanceMetrics.processingTimeMs !== "number")
        return false;
      if (typeof result.performanceMetrics.computationalIntensity !== "string")
        return false;
      if (
        !["low", "medium", "high"].includes(
          result.performanceMetrics.computationalIntensity,
        )
      )
        return false;
    }
