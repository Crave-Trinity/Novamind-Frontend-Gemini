...
describe("Treatment runtime validators", () => {
  ...
  it("TreatmentResponseRequestValidator validates correct TreatmentResponseRequest objects", () => {
    ...
    expect(TreatmentResponseRequestValidator.isValid(invalidRequest)).toBe(
      false,
    );
  ...
  it("TreatmentDetailsValidator validates correct TreatmentDetails objects", () => {
    ...
    expect(TreatmentDetailsValidator.isValid(validMedicationDetails)).toBe(
      true,
    );
    expect(TreatmentDetailsValidator.isValid(validPsychotherapyDetails)).toBe(
      true,
    );
    expect(TreatmentDetailsValidator.isValid(validNeuromodulationDetails)).toBe(
      true,
    );
  ...
  it("TreatmentDetailsValidator.isValidMedication validates medication details", () => {
    ...
    expect(TreatmentDetailsValidator.isValidMedication(validMedication)).toBe(
      true,
    );
    expect(TreatmentDetailsValidator.isValidMedication(invalidMedication)).toBe(
      false,
    );
  });
  ...
  it("ClinicalPredictionDataValidator validates correct ClinicalPredictionData objects", () => {
    const validClinicalData = {
      ...
      illnessDuration: 6,
      previousTreatmentResponses: [
        { treatmentType: "SSRI", response: "partial" },
      ],
    ...
    expect(ClinicalPredictionDataValidator.isValid(validClinicalData)).toBe(
      true,
    );
    expect(ClinicalPredictionDataValidator.isValid(invalidClinicalData)).toBe(
      false,
    );
  ...
  it("GeneticPredictionDataValidator validates correct GeneticPredictionData objects", () => {
    ...
    expect(
      GeneticPredictionDataValidator.isValidMetabolizerStatus(
        validGeneticData.metabolizerStatus,
      ),
    ).toBe(true);
    expect(
      GeneticPredictionDataValidator.isValidMetabolizerStatus(
  ...
  it("TreatmentResponsePredictionValidator validates correct TreatmentResponsePrediction objects", () => {
    const validPrediction = {
      ...
      sideEffectRisks: [
        { effect: "nausea", probability: 0.2, severity: "mild" },
      ],
    ...
    expect(TreatmentResponsePredictionValidator.isValid(validPrediction)).toBe(
      true,
    );
    expect(
      TreatmentResponsePredictionValidator.isValid(invalidPrediction),
    ).toBe(false);
  ...
  it("TreatmentComparisonResultValidator validates correct TreatmentComparisonResult objects", () => {
    ...
    expect(TreatmentComparisonResultValidator.isValid(validComparison)).toBe(
      true,
    );
    expect(TreatmentComparisonResultValidator.isValid(invalidComparison)).toBe(
      false,
    );
