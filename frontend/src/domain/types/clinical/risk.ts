...
export interface DomainRisk {
  domain: 'suicide' | 'self_harm' | 'harm_to_others' | 'psychosis' | 'substance_use' | 'functional_decline' | 'treatment_resistance' | 'medical_complication';
  riskLevel: RiskLevel;
  confidenceScore: number; // 0-1 representing confidence level
  evidence: string[];
...
export interface ContributingFactor {
  id: string;
  name: string;
  category: 'demographic' | 'clinical' | 'psychological' | 'social' | 'environmental' | 'neurobiological';
...
export interface NeuralRiskCorrelate {
  ...
  abnormalityType: 'hyperactivity' | 'hypoactivity' | 'connectivity_disruption' | 'structural_abnormality' | 'neurochemical_imbalance';
  confidenceScore: number; // 0-1 representing confidence level
  description?: string;
...
export interface RiskTimelineEvent {
  ...
  patientId: string;
  timestamp: string;
  eventType: 'assessment' | 'symptom_change' | 'life_event' | 'treatment_change' | 'biometric_alert';
...
export interface BiometricRiskAlert {
  ...
  patientId: string;
  timestamp: string;
  dataSource: 'heart_rate' | 'sleep' | 'activity' | 'location' | 'social' | 'digital_phenotyping' | 'self_report';
...
export type RiskTimelineState = 
  ...
  | { status: 'loading' }
  | { status: 'error'; error: Error }
  | { status: 'success'; events: RiskTimelineEvent[]; timeline: ProcessedRiskTimeline };
...
export const RiskAssessmentOps = {
  ...
  getDomainRisk: (assessment: RiskAssessment | null | undefined, domain: string): DomainRisk | undefined => {
  ...
  calculateRiskScore: (assessment: RiskAssessment): number => {
    ...
    if (!contributingFactors.isEmpty()) {
      const factorScore = contributingFactors
