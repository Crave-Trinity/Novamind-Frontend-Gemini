...
export const biometricService = {
  fetchBiometricData: vi.fn().mockResolvedValue({
    ...
    sleepData: [
      { date: '2023-04-01', duration: 7.5, quality: 0.85, deepSleepPercentage: 0.25 },
      { date: '2023-04-02', duration: 8.0, quality: 0.9, deepSleepPercentage: 0.3 }
    ],
    cortisol: [
...
export const treatmentService = {
  fetchTreatmentData: vi.fn().mockResolvedValue({
    currentMedications: [
      ...
      { id: 'med-2', name: 'Lorazepam', dosage: '0.5mg', frequency: 'as needed' }
    ],
    therapySessions: [
      { id: 'therapy-1', date: '2023-04-01', type: 'CBT', duration: 60, notes: 'Focused on anxiety triggers' },
      { id: 'therapy-2', date: '2023-04-08', type: 'CBT', duration: 60, notes: 'Exposure exercise planning' }
    ],
    adherence: {
      medications: 0.92,
      therapy: 1.0,
  ...
  getTreatmentRecommendations: vi.fn().mockResolvedValue([
    {
      ...
      details: 'Consider increasing dosage to 75mg after week 4 if minimal response',
      evidenceLevel: 'A',
      confidence: 0.85
    },
    {
...
export const neuralActivityService = {
  fetchNeuralData: vi.fn().mockResolvedValue({
    ...
    temporalDynamics: [
      { timestamp: 1617235200000, regionId: 'prefrontal-cortex', activity: 0.78 },
      { timestamp: 1617235800000, regionId: 'prefrontal-cortex', activity: 0.72 },
  ...
  simulateActivationPattern: vi.fn().mockImplementation((pattern) => {
    return Promise.resolve({
      regions: [
        { id: 'prefrontal-cortex', activity: pattern === 'anxiety' ? 0.55 : 0.75, connectivity: 0.68 },
        { id: 'amygdala', activity: pattern === 'anxiety' ? 0.95 : 0.65, connectivity: 0.62 },
        { id: 'hippocampus', activity: pattern === 'anxiety' ? 0.45 : 0.72, connectivity: 0.71 }
      ],
