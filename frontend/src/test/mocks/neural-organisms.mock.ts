...
export const mockNeuralControlPanel = vi.fn().mockImplementation(
  ({
    patientId,
    activeRegions = [],
    onRegionSelect,
...
export const mockClinicalMetricsPanel = vi.fn().mockImplementation(
  ({
    patientId,
    metrics = {},
    showSymptoms = true,
    ...
    return React.createElement(
      ...
      'Clinical Metrics',
      showSymptoms && React.createElement('div', { 'data-testid': 'symptom-metrics' }, 'Symptom Metrics'),
      showTreatments && React.createElement('div', { 'data-testid': 'treatment-metrics' }, 'Treatment Metrics'),
      showPredictions && React.createElement('div', { 'data-testid': 'prediction-metrics' }, 'Prediction Metrics'),
      showRisks && React.createElement('div', { 'data-testid': 'risk-metrics' }, 'Risk Metrics')
    );
  }
);

export const mockBiometricMonitorPanel = vi.fn().mockImplementation(
  ({
    patientId,
    data = {},
    showAlerts = true,
    ...
    return React.createElement(
      ...
      'Biometric Monitor',
      showAlerts && React.createElement(
        'div',
        {
          'data-testid': 'alert-panel',
          onClick: () => onAlertAcknowledge && onAlertAcknowledge('alert-1')
        },
        'Active Alerts'
      )
    );
