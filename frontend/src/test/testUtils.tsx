...
class MockApiClient implements ApiClient {
  ...
  async updateBrainModel(model: Partial<BrainModel>): Promise<Result<BrainModel>> {
    return { 
      success: true, 
      data: { 
        ...await this.fetchBrainModel(model.id || 'default-id').then(r => r.data),
        ...model
      } 
    };
  }
  ...
  async predictTreatmentResponse(request: TreatmentRequest): Promise<Result<TreatmentResponsePrediction[]>> {
  ...
  async getTemporalProjection(patientId: string, days: number): Promise<Result<TemporalActivationSequence>> {
  ...
  private createMockActivityState(entityId: string, activity: number): NeuralActivityState {
    return {
      entityId,
      entityType: 'region',
      timestamp: Date.now(),
      rawActivity: activity,
      activationLevel: activity < 0.3 
        ? ActivationLevel.LOW 
        : activity < 0.6 
...
function AllProviders({ 
  ...
  const highlightRegion = React.useCallback((id: string, highlight: boolean) => {
...
function createMockBrainRegions(count: number = 5): BrainRegion[] {
  ...
  const hemispheres: Array<'left' | 'right' | 'central'> = ['left', 'right', 'central'];
  ...
  for (let i = 0; i < count; i++) {
    regions.push({
      ...
      connections: Array.from({ length: Math.floor(Math.random() * 3) + 1 }).map(() => 
        `region-${Math.floor(Math.random() * count) + 1}`
      ),
      activityLevel: Math.random(),
      volumeMl: Math.random() * 20 + 5,
      isActive: Math.random() > 0.3,
      riskFactor: Math.random(),
      clinicalSignificance: Math.random() > 0.5 ? 'High relevance in mood regulation' : undefined,
      hemisphereLocation: hemispheres[Math.floor(Math.random() * hemispheres.length)],
...
function createMockNeuralConnections(count: number = 10): NeuralConnection[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    connections.push({
      ...
      directionality: directionalities[Math.floor(Math.random() * directionalities.length)],
      activityLevel: Math.random(),
  ...
  for (let day = 0; day < days; day++) {
    ...
    for (let hour = 0; hour < 24; hour += 6) {
      ...
      readings.push({
        ...
        heartRate: baseline.heartRate + circadianFactor * 10 + weeklyFactor * 5 + (Math.random() * 8 - 4),
        respirationRate: baseline.respirationRate + circadianFactor * 2 + weeklyFactor + (Math.random() * 3 - 1.5),
        cortisol: baseline.cortisol + (hour < 12 ? 8 : 0) + weeklyFactor * 3 + (Math.random() * 4 - 2),
        sleepQuality: hour === 6 ? baseline.sleepQuality + weeklyFactor * 0.2 + (Math.random() * 0.3 - 0.15) : 0,
        socialActivity: hour >= 12 && hour <= 18 ? baseline.socialActivity + weeklyFactor * 0.2 + (Math.random() * 0.4 - 0.2) : 0.1,
        digitalPhenotype: {
          screenTime: (hour >= 8 && hour <= 22 ? 0.8 : 0.1) + weeklyFactor * 0.3 + (Math.random() * 0.4 - 0.2),
          messageFrequency: (hour >= 10 && hour <= 20 ? 10 : 1) + weeklyFactor * 5 + (Math.random() * 8 - 4),
          typingSpeed: 60 + weeklyFactor * 10 + (Math.random() * 20 - 10),
          appUsagePatterns: {
            'social': (hour >= 12 && hour <= 20 ? 0.6 : 0.2) + weeklyFactor * 0.2 + (Math.random() * 0.3 - 0.15),
            'productivity': (hour >= 9 && hour <= 17 ? 0.7 : 0.1) + weeklyFactor * 0.1 + (Math.random() * 0.2 - 0.1),
            'entertainment': (hour >= 18 && hour <= 23 ? 0.8 : 0.2) + weeklyFactor * 0.3 + (Math.random() * 0.3 - 0.15),
...
const customMatchers = {
  toHaveNeuralStructure: (received: any, expectedKeys: string[]) => {
    ...
    return {
      ...
      message: () => `Expected object to have neural structure with keys: ${expectedKeys.join(', ')}. 
        Missing keys: ${missingKeys.join(', ')}`
    };
  },
  
  toBeValidBrainRegion: (received: any) => {
    const requiredKeys = ['id', 'name', 'position', 'activityLevel', 'isActive', 'hemisphereLocation', 'dataConfidence'];
