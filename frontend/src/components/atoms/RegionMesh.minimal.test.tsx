...
vi.mock('@react-three/drei', () => ({
  Sphere: ({ children, position, args, onClick, onPointerOver, onPointerOut }: any) => (
    <div 
  ...
  MeshDistortMaterial: ({ color, emissive, emissiveIntensity, distort, speed, opacity }: any) => (
    <div 
...
describe('RegionMesh', () => {
  ...
  it('renders with neural precision', () => {
    ...
    expect(JSON.parse(sphereElement.getAttribute('data-position') || '[]')).toEqual([0, 0, 0]);
    expect(JSON.parse(sphereElement.getAttribute('data-args') || '[]')[0]).toEqual(1.5);
