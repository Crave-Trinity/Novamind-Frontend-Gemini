...
export function validateBrainModelData(data: unknown): Result<BrainModel, Error> {
    if (isBrainModel(data)) { // Use the domain type guard
        return Ok(data);
    }
...
export function validateRenderMode(mode: unknown): Result<RenderMode, Error> {
    if (isValidRenderMode(mode)) { // Use the domain type guard
        return Ok(mode);
    }
    return Err(new Error(`Invalid RenderMode value. Must be one of: ${Object.values(RenderMode).join(', ')}`));
}
