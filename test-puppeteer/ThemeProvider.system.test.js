...
(async () => {
  ...
  try {
    ...
    assert.strictEqual(
      isDarkModeApplied,
      true,
      '❌ FAILURE (Test 1): HTML element should have "dark" class after emulating dark preference.'
    );
    ...
    console.log(
      `[Puppeteer] Verified prefers-color-scheme: light in browser context: ${prefersLight}`
    );
    assert.strictEqual(
      prefersLight,
      true,
      '❌ FAILURE: Emulation of light mode failed in browser context.'
    );
    ...
    assert.strictEqual(
      ...
      false,
      '❌ FAILURE (Test 2): HTML element should NOT have "dark" class after emulating light preference.'
    );
