...
def mock_symptom_forecasting_service():
    ...
    service = AsyncMock()
    service.forecast_symptoms = AsyncMock(return_value={
        "forecasts": {
            "anxiety": [5, 4, 3, 4, 5],
        ...
        "trending_symptoms": [
            {"symptom": "anxiety", "trend": "stable", "insight_text": "Anxiety levels remain stable", "importance": 0.7}
        ],
        "risk_alerts": []
    })
    return service
...
def mock_biometric_correlation_service():
    ...
    service.analyze_correlations = AsyncMock(return_value={
        ...
        "lag_correlations": [
            {"biometric": "sleep_quality", "lag_days": 2, "mental_health_indicator": "depression", "correlation": 0.65}
        ],
        "insights": [
            {"insight_text": "Heart rate strongly correlates with anxiety levels", "importance": 0.8}
        ],
        "monitoring_plan": {
...
def mock_pharmacogenomics_service():
    ...
    service = AsyncMock()
    service.predict_medication_responses = AsyncMock(return_value={
        ...
        "recommendations": {
            ...
            "alternative_recommendations": [
                {"medication": "sertraline", "rationale": "Moderate probability of good response"}
            ]
        },
        "genetic_insights": [
            {"gene": "CYP2D6", "variant": "Normal metabolizer", "insight": "Standard dosing appropriate"}
        ]
    })
    return service
...
@pytest.fixture
def integration_service(mock_symptom_forecasting_service, mock_biometric_correlation_service, mock_pharmacogenomics_service):
...
def patient_data():
    ...
    return {
        ...
        "biometric_data": [
            {"date": "2023-01-01", "heart_rate": 75, "sleep_quality": 0.7, "activity_level": 0.6},
            {"date": "2023-01-02", "heart_rate": 80, "sleep_quality": 0.6, "activity_level": 0.5},
            {"date": "2023-01-03", "heart_rate": 70, "sleep_quality": 0.8, "activity_level": 0.7},
            {"date": "2023-01-04", "heart_rate": 72, "sleep_quality": 0.7, "activity_level": 0.6},
            {"date": "2023-01-05", "heart_rate": 74, "sleep_quality": 0.7, "activity_level": 0.6}
        ],
...
async def test_generate_integrated_recommendations(integration_service):
    ...
    results = [
        {
            "symptom_forecasting": {
                "forecasts": {"anxiety": [5, 4, 3, 4, 5]}
            },
            "symptom_forecasting_insights": [
                {"source": "symptom_forecasting", "insight": "Anxiety levels remain stable", "importance": 0.7}
            ]
        },
        {
            ...
            "biometric_correlation_insights": [
                {"source": "biometric_correlation", "insight": "Heart rate correlates with anxiety", "importance": 0.8}
            ]
        },
        {
            "pharmacogenomics": {
                "recommendations": {"primary_recommendations": [{"medication": "fluoxetine"}]}
            },
            "pharmacogenomics_insights": [
                {"source": "pharmacogenomics", "insight": "Good predicted response to fluoxetine", "importance": 0.9}
            ]
        }
    ]
    
    # Generate integrated recommendations
    recommendations = await integration_service._generate_integrated_recommendations(results)
