...
# -*- coding: utf-8 -*-
"""
Fixed Direct Test Execution for PHI Audit Tests
...
def test_audit_with_clean_files(temp_dir):
    ...
    # Create a regular app directory (not clean_app)
...
""")
    ...
    with patch('scripts.run_hipaa_phi_audit.logger', mock_logger):
        ...
        phi_findings = [f for f in auditor.findings["code_phi"] if not f.get("is_allowed", False)]
        if len(phi_findings) > 0:
            print("❌ Failed: PHI was found in clean files")
            return False
...
def test_phi_detector_ssn_pattern():
    ...
    if not result:
        print("❌ Failed: No PHI detected in text with SSN")
        return False
    
    # Check for SSN in results - handling both dict and object interfaces
    ssn_detected = False
    for match in result:
        # Try different access methods since we're not sure of the exact structure
        try:
            ...
            elif hasattr(match, 'content') and hasattr(match, 'type'):
            ...
            if "123-45-6789" in content:
                ssn_detected = True
                if match_type != "SSN" and match_type != "unknown":
                    print(f"❌ Warning: Pattern identified as {match_type}, expected SSN")
                break
        except Exception as e:
            print(f"❌ Error accessing match properties: {e}")
