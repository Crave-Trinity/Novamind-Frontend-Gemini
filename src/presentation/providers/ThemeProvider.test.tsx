...
describe('ThemeProvider', () => {
  ...
  beforeEach(() => {
    ...
    const mockMediaQueryListObject: {
        ...
        addListener: vi.fn((listener) => { mediaQueryChangeListener = listener; }), // Deprecated fallback
        removeListener: vi.fn((listener) => { if (mediaQueryChangeListener === listener) { mediaQueryChangeListener = null;} }), // Deprecated fallback
  ...
  it('follows system theme when set to system', async () => {
    ...
    await waitFor(() => {
        ...
        expect(document.documentElement.classList.contains('light')).toBe(true);
    }, { timeout: 1000 }); // Increased timeout slightly more
    ...
    expect(document.documentElement.classList.contains('dark')).toBe(false);
     ...
     await waitFor(() => {
         expect(document.documentElement.classList.contains('dark')).toBe(true);
     }, { timeout: 500 }); // Increase timeout slightly
