...
const mockProps = {
  ...
  profile: {
    ...
    clinicalData: { // Ensure nested objects match their types
      diagnoses: [{
        id: 'diag-1',
        code: "F33.1",
        codingSystem: "ICD-10",
      ...
      symptoms: [{ id: 'symp-1', name: "Anhedonia", severity: 7, category: 'affective', frequency: 'daily', impact: 'moderate', progression: 'stable' }],
      medications: [{ id: 'med-1', name: "Sertraline", dosage: "100mg", classification: 'SSRI', frequency: 'daily', route: 'oral', startDate: new Date().toISOString() }],
      psychometricAssessments: [{ id: 'pa-1', name: "PHQ-9", date: new Date().toISOString(), scores: [{ subscaleName: 'Total', rawScore: 14, interpretation: 'Moderate depression', clinicalSignificance: true }], interpretation: 'Moderate depression' }],
      // assessmentScores property removed from ClinicalData as it's top-level in DigitalTwinProfile
      medicalHistory: [{ id: 'mh-1', condition: "Hypertension", type: 'cardiovascular', status: 'chronic', relevanceToNeuralHealth: 'moderate' }], // Corrected type
    } as ClinicalData, // Keep outer assertion for now
    treatmentData: { // Correct Treatment structure
      currentTreatments: [{
        id: "tx-1",
        name: "Sertraline",
    ...
    riskAssessments: [ // Added missing property with basic structure matching RiskAssessment type
        { id: 'ra-1', type: 'suicide', level: 'low', factors: ['hopelessness'], date: new Date().toISOString(), assessor: 'Dr. AI', notes: 'Monitor closely' }
    ], // Removed 'as RiskAssessment[]'
    assessmentScores: [ // Moved to top level profile, ensure structure matches AssessmentScore
        { id: 'as-1', type: "PHQ9", score: 14, maxScore: 27, clinicalSignificance: 'moderate', date: new Date().toISOString() },
        { id: 'as-2', type: "GAD7", score: 11, maxScore: 21, clinicalSignificance: 'moderate', date: new Date().toISOString() },
    ], // Removed 'as AssessmentScore[]'
    treatmentPlan: null, // Added missing property (can be null or mock object)
    biomarkers: [ // Added missing property with example structure matching Biomarker type
        { id: 'bio-1', name: 'genetic_marker_CYP2D6', value: '*1/*4', type: 'genetic', date: new Date().toISOString(), interpretation: 'Intermediate metabolizer', referenceRange: 'N/A', relevance: 'Pharmacogenomic' }
