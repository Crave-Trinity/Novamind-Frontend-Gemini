...
import { SymptomNeuralMapping, DiagnosisNeuralMapping, NeuralActivationPattern } from "@/domain/models/brain/mapping/brain-mapping";
describe("calculateNeuralActivation", () => {
  ...
  const mockRegions: BrainRegion[] = [
    ...
    { id: "r1", name: "Region 1", position: { x: 0, y: 0, z: 0 }, activityLevel: 0, isActive: false, color: '', connections: [], hemisphereLocation: 'left', dataConfidence: 1 },
    { id: "r2", name: "Region 2", position: { x: 1, y: 1, z: 1 }, activityLevel: 0, isActive: false, color: '', connections: [], hemisphereLocation: 'right', dataConfidence: 1 },
    { id: "r3", name: "Region 3", position: { x: -1, y: -1, z: -1 }, activityLevel: 0, isActive: false, color: '', connections: [], hemisphereLocation: 'central', dataConfidence: 1 },
  ];

  const mockSymptomMappings: SymptomNeuralMapping[] = [
    {
      symptomId: "s1", symptomName: "Anxiety", category: "Emotional", evidenceQuality: "established", contributingFactors: [],
      activationPatterns: [
        { regionIds: ["r1"], intensity: 0.8, confidence: 0.9, timeScale: "acute", connectivity: { increasedPathways: [], decreasedPathways: [] } }, // r1 activation = 0.8 * 0.9 = 0.72
        { regionIds: ["r2"], intensity: 0.5, confidence: 0.7, timeScale: "acute", connectivity: { increasedPathways: [], decreasedPathways: [] } }, // r2 activation = 0.5 * 0.7 = 0.35
      ]
    },
    {
      symptomId: "s2", symptomName: "Insomnia", category: "Sleep", evidenceQuality: "probable", contributingFactors: [],
      activationPatterns: [
        { regionIds: ["r2"], intensity: 0.6, confidence: 0.8, timeScale: "chronic", connectivity: { increasedPathways: [], decreasedPathways: [] } }, // r2 activation = 0.6 * 0.8 = 0.48
      ]
    }
  ];
  ...
  const mockDiagnosisMappings: DiagnosisNeuralMapping[] = [
     {
       diagnosisId: "d1", diagnosisName: "MDD", codingSystem: "ICD-10", evidenceQuality: "established",
       activationPatterns: [
         { regionIds: ["r1"], intensity: 0.7, confidence: 0.85, timeScale: "chronic", connectivity: { increasedPathways: [], decreasedPathways: [] } }, // r1 activation = 0.7 * 0.85 = 0.595
         { regionIds: ["r3"], intensity: 0.4, confidence: 0.6, timeScale: "chronic", connectivity: { increasedPathways: [], decreasedPathways: [] } }, // r3 activation = 0.4 * 0.6 = 0.24
       ]
     }
  ];
  ...
  const mockActiveSymptoms: Symptom[] = [
    ...
    { id: "s1", name: "Anxiety", severity: 7, onsetDate: new Date().toISOString(), category: 'affective', frequency: 'daily', impact: 'moderate', progression: 'stable' },
    { id: "s2", name: "Insomnia", severity: 5, onsetDate: new Date().toISOString(), category: 'behavioral', frequency: 'daily', impact: 'moderate', progression: 'stable' }, // Changed category
  ];

  const mockActiveDiagnoses: Diagnosis[] = [
     // Corrected: Use ISO string for dates. Added missing required properties.
     { id: "d1", name: "MDD", code: "F33.1", codingSystem: "ICD-10", severity: "moderate", onsetDate: new Date().toISOString(), diagnosisDate: new Date().toISOString(), status: 'active' }
  ];
  ...
  it("should calculate activation based on symptoms only", () => {
    ...
    expect(activationMap.get("r2")).toBeCloseTo(Math.sqrt(Math.pow(0.7 * 0.5 * 0.7, 2) + Math.pow(0.5 * 0.6 * 0.8, 2))); // approx 0.343
    expect(activationMap.get("r3")).toBe(0);
  });

  it("should calculate activation based on diagnoses only", () => {
    const result = calculateNeuralActivation(mockRegions, [], [], mockDiagnosisMappings, mockActiveDiagnoses);
    expect(result.success).toBe(true);
  ...
  it("should combine symptom and diagnosis activations quadratically", () => {
    const result = calculateNeuralActivation(mockRegions, mockSymptomMappings, mockActiveSymptoms, mockDiagnosisMappings, mockActiveDiagnoses);
    expect(result.success).toBe(true);
    if (!result.success) throw result.error;
    const activationMap = result.value; // Corrected back to .value
    ...
    expect(activationMap.get("r1")).toBeCloseTo(Math.sqrt(Math.pow(r1_sym_act, 2) + Math.pow(r1_dia_act, 2))); // approx 0.6176
  ...
  it("should return 0 activation if no mappings match", () => {
     ...
     const nonMatchingSymptoms: Symptom[] = [{ id: "s99", name: "Unknown", severity: 8, onsetDate: new Date().toISOString(), category: 'somatic', frequency: 'constant', impact: 'severe', progression: 'worsening' }];
   ...
   it("should cap activation at 1.0", () => {
     // Corrected: Use ISO string for dates. Added missing required properties.
     // Corrected: Use ISO string for dates, remove lastUpdated. Added missing required properties.
     const highImpactSymptom: Symptom[] = [{ id: "s1", name: "Anxiety", severity: 10, onsetDate: new Date().toISOString(), category: 'affective', frequency: 'constant', impact: 'severe', progression: 'stable' }];
     const highImpactMapping: SymptomNeuralMapping[] = [{
       ...
       activationPatterns: [
         ...
         { regionIds: ["r1"], intensity: 0.9, confidence: 0.9, timeScale: "acute", connectivity: { increasedPathways: [], decreasedPathways: [] } },
         { regionIds: ["r1"], intensity: 0.8, confidence: 0.8, timeScale: "acute", connectivity: { increasedPathways: [], decreasedPathways: [] } },
       ]
     }];
