...
describe("BrainModelService Runtime Validation", () => {
  ...
  describe("validateBrainModel", () => {
    it("returns success for valid BrainModel objects", () => {
      ...
      const validRegion: BrainRegion = { id: "region-1", name: "Prefrontal Cortex", position: { x: 0, y: 0, z: 0 }, color: "#FF0000", connections: ["connection-1"], hemisphereLocation: "left", dataConfidence: 0.9, activityLevel: 0.8, isActive: true }; // Keep required properties
      const validConnection: NeuralConnection = { id: "connection-1", sourceId: "region-1", targetId: "region-2", strength: 0.7, type: "functional", directionality: "unidirectional", activityLevel: 0.6, dataConfidence: 0.85 }; // Corrected mock
      const validModel: BrainModel = { id: "model-123", regions: [validRegion], connections: [validConnection], version: "1", patientId: "patient-test", scan: { id: 'scan-test', patientId: 'patient-test', scanDate: new Date().toISOString(), scanType: 'fMRI', dataQualityScore: 0.9 }, timestamp: new Date().toISOString(), processingLevel: "raw", lastUpdated: new Date().toISOString() }; // Corrected mock
  ...
  describe("isBrainRegion", () => {
    it("returns true for valid BrainRegion objects", () => {
      const validRegion: BrainRegion = { id: "region-1", name: "Prefrontal Cortex", position: { x: 0, y: 0, z: 0 }, color: "#FF0000", connections: [], hemisphereLocation: "left", dataConfidence: 0.9, activityLevel: 0.8, isActive: true }; // Keep required properties
  ...
  describe("validateNeuralConnection", () => {
    ...
    it("includes the field path in error messages when provided", () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) expect((result.error as ValidationError)?.field).toBe("connectionField.id"); // Use imported ValidationError for cast
    });
  });
});
