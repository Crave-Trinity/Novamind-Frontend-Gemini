 ...
 */
export function validatePartialBrainViewState(
  data: unknown,
): Result<PartialBrainViewState, Error> {
  if (typeof data !== "object" || data === null) {
    return Err(
      new Error("Invalid PartialBrainViewState: Input must be an object."),
    );
  }
  ...
  if (
    viewState.rotationX !== undefined &&
    typeof viewState.rotationX !== "number"
  ) {
    return Err(
      new Error("Invalid PartialBrainViewState: rotationX must be a number."),
    );
  }
  if (
    viewState.rotationY !== undefined &&
    typeof viewState.rotationY !== "number"
  ) {
    return Err(
      new Error("Invalid PartialBrainViewState: rotationY must be a number."),
    );
  }
  if (
    viewState.rotationZ !== undefined &&
    typeof viewState.rotationZ !== "number"
  ) {
    return Err(
      new Error("Invalid PartialBrainViewState: rotationZ must be a number."),
    );
  }
  if (
    ...
    return Err(
      new Error(
        "Invalid PartialBrainViewState: zoom must be a positive number.",
      ),
    );
  }
  if (
    ...
    return Err(
      new Error(
        "Invalid PartialBrainViewState: highlightedRegions must be an array of strings.",
      ),
    );
  }
  if (
    viewState.visiblePathways !== undefined &&
    typeof viewState.visiblePathways !== "boolean"
  ) {
    return Err(
      new Error(
        "Invalid PartialBrainViewState: visiblePathways must be a boolean.",
      ),
    );
  }
  if (
    viewState.renderMode !== undefined &&
    !isValidRenderMode(viewState.renderMode)
  ) {
    return Err(
      new Error(
