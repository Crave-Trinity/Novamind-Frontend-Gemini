...
const validTimeScales: TimeScale[] = [
  "momentary",
  "hourly",
  "daily",
  "weekly",
...
function isTimeScale(value: unknown): value is TimeScale {
  return (
    typeof value === "string" && validTimeScales.includes(value as TimeScale)
  );
}

function isRecordOfPositiveNumbers(
  record: unknown,
): record is Record<TimeScale, number> {
 ...
 */
export function validatePartialTemporalConfig(
  config: unknown,
): Result<PartialTemporalConfig, Error> {
  if (typeof config !== "object" || config === null) {
    return Err(
      new Error("Invalid Partial<TemporalConfig>: Input must be an object."),
    );
  }
  ...
  if (
    cfg.timeScales !== undefined &&
    (!Array.isArray(cfg.timeScales) || !cfg.timeScales.every(isTimeScale))
  ) {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: timeScales must be an array of valid TimeScale values.",
      ),
    );
  }
  if (
    cfg.patternRecognitionThreshold !== undefined &&
    (typeof cfg.patternRecognitionThreshold !== "number" ||
      cfg.patternRecognitionThreshold < 0 ||
      cfg.patternRecognitionThreshold > 1)
  ) {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: patternRecognitionThreshold must be a number between 0.0 and 1.0.",
      ),
    );
  }
  if (
    cfg.criticalTransitionSensitivity !== undefined &&
    (typeof cfg.criticalTransitionSensitivity !== "number" ||
      cfg.criticalTransitionSensitivity < 0 ||
      cfg.criticalTransitionSensitivity > 1)
  ) {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: criticalTransitionSensitivity must be a number between 0.0 and 1.0.",
      ),
    );
  }
  if (
    cfg.historyLength !== undefined &&
    !isRecordOfPositiveNumbers(cfg.historyLength)
  ) {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: historyLength must be a Record mapping TimeScale to positive numbers.",
      ),
    );
  }
  if (
    cfg.samplingRate !== undefined &&
    !isRecordOfPositiveNumbers(cfg.samplingRate)
  ) {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: samplingRate must be a Record mapping TimeScale to positive numbers.",
      ),
    );
  }
  if (cfg.periodicity !== undefined && typeof cfg.periodicity !== "boolean") {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: periodicity must be a boolean.",
      ),
    );
  }
  if (
    cfg.anomalyDetection !== undefined &&
    typeof cfg.anomalyDetection !== "boolean"
  ) {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: anomalyDetection must be a boolean.",
      ),
    );
  }
  if (cfg.filterNoise !== undefined && typeof cfg.filterNoise !== "boolean") {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: filterNoise must be a boolean.",
      ),
    );
  }
  if (
    cfg.smoothingFactor !== undefined &&
    (typeof cfg.smoothingFactor !== "number" ||
      cfg.smoothingFactor < 0 ||
      cfg.smoothingFactor > 1)
  ) {
    return Err(
      new Error(
        "Invalid Partial<TemporalConfig>: smoothingFactor must be a number between 0.0 and 1.0.",
      ),
    );
  }
...
export function validateTimeScale(
  timeScale: unknown,
): Result<TimeScale, Error> {
  if (isTimeScale(timeScale)) {
    return Ok(timeScale);
  }
  return Err(
    new Error(
      `Invalid TimeScale: Must be one of ${validTimeScales.join(", ")}.`,
    ),
  );
}
...
export function validateMetricIds(metricIds: unknown): Result<string[], Error> {
  ...
  return Err(
    new Error("Invalid metricIds: Input must be an array of strings."),
  );
}
 ...
 */
export function validateAnalysisParameters(
  params: unknown,
): Result<AnalysisParameters, Error> {
  // TODO: Implement detailed validation logic when AnalysisParameters type is defined
  if (typeof params !== "object" || params === null) {
    return Err(
      new Error("Invalid AnalysisParameters: Input must be an object."),
    );
  }
