...
from app.domain.entities.clinical_note import (
    ClinicalNote, DiagnosisEntry, MedicationEntry, NoteStatus, NoteType
)
...
class ClinicalDocumentationService:
    ...
    async def create_note(self,
                         ...
                         content: Optional[str] = None) -> ClinicalNote:
        """
    ...
    async def update_note_content(self,
                                note_id: UUID,
                                new_content: str,
                                provider_id: UUID) -> ClinicalNote:
        """
    ...
    async def add_diagnosis(self,
                          note_id: UUID,
                          code: str,
                          description: str,
                          provider_id: UUID,
                          primary: bool = False,
                          notes: Optional[str] = None) -> ClinicalNote:
        """
    ...
    async def add_medication_entry(self,
                                 note_id: UUID,
                                 name: str,
                                 dosage: str,
                                 frequency: str,
    ...
    async def get_patient_notes_chronological(self,
                                           patient_id: UUID,
                                           limit: int = 100,
                                           offset: int = 0) -> List[ClinicalNote]:
        """
    ...
    async def get_notes_by_diagnosis_code(self,
                                        code: str,
                                        limit: int = 100,
                                        offset: int = 0) -> List[ClinicalNote]:
        """
        ...
        while True:
            batch = await self._note_repo.list_by_status(NoteStatus.SIGNED, batch_size, batch_offset)
            if not batch:
                break
