...
from app.domain.exceptions import (
    AuthorizationError, BusinessRuleViolationError, ValidationError
)
...
class PatientService:
    ...
    async def register_patient(self,
                             ...
                             preferred_provider_id: Optional[UUID] = None) -> Patient:
        """
        ...
        if age < 18:
            raise ValidationError("Patient must be at least 18 years old for adult psychiatry services")
    ...
    async def update_patient_info(self,
                                patient_id: UUID,
                                updated_fields: dict) -> Patient:
        """
    ...
    async def get_patient_care_summary(self, patient_id: UUID) -> dict:
        ...
        if patient.preferred_provider_id:
            preferred_provider = await self._provider_repo.get_by_id(patient.preferred_provider_id)
        ...
        care_summary = {
            ...
            } if preferred_provider else None,
            "recent_appointments": [
                {
                    "id": str(appt.id),
                    "date": appt.appointment_date.isoformat(),
                    "type": appt.appointment_type.name if hasattr(appt, 'appointment_type') else None,
    ...
    async def assign_preferred_provider(self,
                                      patient_id: UUID,
                                      provider_id: UUID) -> Patient:
        """
    ...
    async def get_patients_with_upcoming_appointments(self, 
        ...
        appointments = await self._appointment_repo.list_by_date_range(start_date, end_date)
