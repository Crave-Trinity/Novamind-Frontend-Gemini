...
from app.domain.entities.medication import (
    DosageSchedule, Medication, MedicationStatus, RefillStatus, SideEffectReport
)
...
class MedicationService:
    ...
    async def prescribe_medication(self,
                                  patient_id: UUID,
                                  provider_id: UUID,
                                  name: str,
                                  dosage: str,
                                  ...
                                  refills: int = 0,
                                  check_interactions: bool = True) -> Medication:
        """
    ...
    async def update_prescription(self,
                                medication_id: UUID,
        ...
        if any([dosage, frequency, timing, max_daily]):
            new_dosage = DosageSchedule(
                ...
                max_daily=max_daily if max_daily is not None else medication.dosage_schedule.max_daily
            )
        ...
        if refills is not None:
            ...
            medication.refill_status = RefillStatus.AVAILABLE if refills > 0 else RefillStatus.EXPIRED
            medication.updated_at = datetime.utcnow()
        
        # Save to repository
        return await self._medication_repo.update(medication)
    
    async def discontinue_medication(self,
                                   medication_id: UUID,
                                   reason: Optional[str] = None) -> Medication:
        """
    ...
    async def approve_refill(self,
                           medication_id: UUID,
                           refills_granted: int = 1) -> Medication:
        """
    ...
    async def report_side_effect(self,
                               medication_id: UUID,
                               effect: str,
                               severity: int,
                               notes: Optional[str] = None,
                               reported_by: Optional[UUID] = None) -> Medication:
        """
    ...
    async def _check_interactions(self, patient_id: UUID, new_medication_name: str) -> None:
        ...
        for med in active_medications:
            med_name_lower = med.name.lower()
            if med_name_lower in self._known_interactions and new_medication_name in self._known_interactions[med_name_lower]:
                potential_interactions.append(med.name)
