...
from app.domain.entities.provider import (
    Provider, ProviderRole, ProviderSpecialty, Credential, AvailabilitySlot
)
from app.domain.exceptions import (
    CredentialExpiredError, PatientLimitExceededError, ValidationError
)
...
class ProviderService:
    ...
    async def register_provider(self,
                              ...
                              max_patients: Optional[int] = None) -> Provider:
        """
    ...
    async def add_credential(self,
                           ...
                           verification_url: Optional[str] = None) -> Provider:
        """
    ...
    async def add_availability(self,
                             provider_id: UUID,
                             day_of_week: int,
                             start_time: datetime,
                             end_time: datetime,
                             is_telehealth: bool = True,
                             is_in_person: bool = True) -> Provider:
        """
    ...
    async def verify_credentials(self, provider_id: UUID) -> Tuple[bool, List[str]]:
        ...
        for i, credential in enumerate(provider.credentials):
            if credential.is_expired:
                ...
                issues.append(f"Credential {credential.type} from {credential.issuer} expires in {days_left} days")
        
        # Check if prescriber role has DEA number
        if provider.is_prescriber and not provider.dea_number:
            issues.append(f"Provider with role {provider.role.name} requires a DEA number for prescribing")
    ...
    async def get_available_providers(self,
                                    ...
                                    is_telehealth: bool = False) -> List[Provider]:
        """
        ...
        if is_telehealth:
            providers = [
                ...
                if any(slot.is_telehealth and slot.day_of_week == day_of_week for slot in p.availability)
            ]
    ...
    async def check_patient_capacity(self, provider_id: UUID) -> Tuple[bool, Optional[int]]:
        """
