...
class Appointment:
    ...
    def __post_init__(self):
        ...
        if (self.appointment_type == AppointmentType.TELEHEALTH and 
            not self.virtual_meeting_link):
            raise ValueError("Telehealth appointments must have a virtual meeting link")
        
        # Ensure in-person appointments have a location
        if (self.appointment_type == AppointmentType.IN_PERSON and 
            not self.location):
    ...
    def mark_no_show(self) -> None:
        ...
        if self.status in [AppointmentStatus.COMPLETED, AppointmentStatus.CANCELLED]:
            raise ValueError("Cannot mark a completed or cancelled appointment as no-show")
        
        self.status = AppointmentStatus.NO_SHOW
        self.updated_at = datetime.utcnow()
    
    def add_note(self, content: str, created_by: Optional[UUID] = None, is_private: bool = False) -> None:
        """
    ...
    def is_upcoming(self, reference_time: Optional[datetime] = None) -> bool:
        ...
        return (self.start_time > reference_time and 
    ...
    def is_in_progress(self, reference_time: Optional[datetime] = None) -> bool:
        ...
        return (self.start_time <= reference_time <= self.end_time and
                self.status not in [AppointmentStatus.CANCELLED, AppointmentStatus.COMPLETED])