...
class EncryptionService:
    ...
    def encrypt_dict(self, data: Dict[str, Any], sensitive_fields: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            should_encrypt = (
                sensitive_fields is None or 
                key in (sensitive_fields or [])
            )
            ...
            if should_encrypt and isinstance(value, str):
                ...
                encrypted_data[key] = self.encrypt_dict(
                    value, 
                    sensitive_fields
                )
                ...
                encrypted_data[key] = [
                    self.encrypt_dict(item, sensitive_fields) if isinstance(item, dict)
                    else item
                    for item in value
                ]
            else:
    ...
    def decrypt_dict(self, data: Dict[str, Any], encrypted_fields: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            should_decrypt = (
                encrypted_fields is None or 
                key in (encrypted_fields or [])
            )
            ...
            if should_decrypt and isinstance(value, str):
                ...
                decrypted_data[key] = self.decrypt_dict(
                    value, 
                    encrypted_fields
                )
            elif isinstance(value, list):
                ...
                decrypted_data[key] = [
                    self.decrypt_dict(item, encrypted_fields) if isinstance(item, dict)
                    else item
                    for item in value
                ]
            else:
    ...
    def generate_hash(self, data: str, salt: Optional[bytes] = None) -> Tuple[str, bytes]:
        """
