...
class XGBoostSymptomModel:
    ...
    def train(self, 
        ...
        if X_train.shape[0] != y_train.shape[0]:
            raise ValidationError(f"X_train and y_train must have the same number of samples, got {X_train.shape[0]} and {y_train.shape[0]}")
    ...
    async def predict(self, 
        ...
        try:
            ...
            if self.feature_names and X.shape[1] != len(self.feature_names):
                raise ValidationError(f"Input has {X.shape[1]} features, but model expects {len(self.feature_names)}")
