...
class MultiHorizonTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        # Positional encoding
        self.positional_encoding = nn.Parameter(torch.zeros(1, 1000, d_model))  # Max sequence length of 1000
    ...
    def create_masks(self, src_seq: torch.Tensor, tgt_seq: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        tgt_mask = tgt_mask.unsqueeze(0).unsqueeze(1).expand(tgt_seq.shape[0], 1, tgt_seq_len, tgt_seq_len)
        ...
        try:
            ...
            with torch.no_grad():
                ...
                return {
                    ...
                    "all_quantiles": {q: all_preds[:, :, :, i].cpu().numpy() 
                                    for i, q in enumerate(self.quantiles)},
                    "model_type": "transformer"
                }
                
        except Exception as e:
            raise ModelInferenceError(f"Error during transformer model inference: {str(e)}")
