...
class PharmacogenomicsService:
    ...
    def __init__(self, 
        ...
        self.medication_categories = {
            ...
            "atypical_antipsychotic": ["quetiapine", "aripiprazole", "risperidone", "olanzapine", "lurasidone"],
    ...
    async def predict_medication_responses(self, 
                                        patient_id: UUID,
        ...
        try:
            ...
            raise ModelInferenceError(f"Failed to predict medication responses: {str(e)}")
            
    async def _generate_medication_insights(self, 
                                         medication_predictions: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    async def _generate_interaction_recommendations(self, 
        ...
        for gene, gene_interactions in interactions_by_gene.items():
            ...
            for variant, variant_interactions in interactions_by_variant.items():
                medications = [interaction.get("medication") for interaction in variant_interactions]
    ...
    async def _identify_common_side_effects(self, 
                                         side_effect_predictions: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    async def _generate_side_effect_insights(self, 
                                          side_effect_predictions: Dict[str, Dict[str, Any]],
                                          common_effects: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        for effect, data in common_effects.items():
            ...
            if data["genetic_factors"]:
                ...
                gene_text = f"{', '.join(genes[:-1])} and {genes[-1]}" if len(genes) > 1 else genes[0]
        ...
        for medication, prediction in side_effect_predictions.items():
            for effect in prediction.get("high_risk_effects", []):
                ...
                if genetic_factors:
                    gene_text = f"{', '.join(genetic_factors[:-1])} and {genetic_factors[-1]}" if len(genetic_factors) > 1 else genetic_factors[0]
                    insight_text += f", influenced by {gene_text} genetic factors"
    ...
    async def recommend_treatment_plan(self, 
                                     patient_id: UUID,
                                     patient_data: Dict[str, Any],
                                     diagnosis: str,
                                     current_medications: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            diagnosis_medications = {
                "major_depressive_disorder": {
                    "first_line": ["sertraline", "escitalopram", "venlafaxine", "bupropion"],
                    "second_line": ["fluoxetine", "duloxetine", "mirtazapine"],
                    "adjunct": ["aripiprazole", "quetiapine", "lithium"]
                },
                "generalized_anxiety_disorder": {
                    "first_line": ["escitalopram", "sertraline", "venlafaxine", "duloxetine"],
                ...
                "adhd": {
                    "first_line": ["methylphenidate", "amphetamine", "lisdexamfetamine"],
            ...
            if diagnosis not in diagnosis_medications:
                raise ValidationError(f"Diagnosis {diagnosis} not supported for treatment recommendations")
    ...
    async def _generate_treatment_recommendations(self, 
        ...
        for medication, interactions in interaction_lookup.items():
            if interactions and any(rec["medication"] == medication for line_recs in recommendations.values() for rec in line_recs):
