...
class ModelMetrics:
    ...
    def calculate_classification_metrics(
        ...
        if y_prob is not None:
            ...
            # For multiclass
                ...
                try:
                    results["roc_auc"] = float(metrics.roc_auc_score(
    ...
    def calculate_time_series_metrics(
        ...
        if not multivariate:
            ...
            for i in range(n_variables):
                ...
                # MAPE
                mape = np.mean(np.abs((y_true_var - y_pred_var) / 
                                      np.maximum(np.abs(y_true_var), 1e-10))) * 100
    ...
    def calculate_pharmacogenomics_metrics(
        ...
        return {
            ...
            "per_medication_class": class_metrics
        }
        ...
        for i, feature in enumerate(biometric_features):
            ...
            for j, indicator in enumerate(mental_health_indicators):
                feature_metrics[feature][indicator] = {
                    ...
                    "rmse": float(np.sqrt(metrics.mean_squared_error(
                        true_correlations[i, j], predicted_correlations[i, j]
                    )))
                }
