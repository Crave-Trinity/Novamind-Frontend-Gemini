...
class DigitalTwinIntegrationService:
    ...
    def __init__(self, 
        ...
        # Initialize services
        self.symptom_forecasting_service = symptom_forecasting_service or SymptomForecastingService(
            model_dir=symptom_dir
        )
        
        self.biometric_correlation_service = biometric_correlation_service or BiometricCorrelationService(
            model_dir=biometric_dir
        )
        
        self.pharmacogenomics_service = pharmacogenomics_service or PharmacogenomicsService(
            model_dir=pharma_dir
        )
        
        logging.info("Digital Twin Integration Service initialized")
        
    async def generate_comprehensive_patient_insights(self, 
                                                   patient_id: UUID,
        ...
        try:
            ...
            if len(results) > 1 and not all(isinstance(r, Exception) for r in results):
                insights["integrated_recommendations"] = await self._generate_integrated_recommendations(results)
                
            return insights
            
        except Exception as e:
            logging.error(f"Error generating comprehensive patient insights: {str(e)}")
            raise ModelInferenceError(f"Failed to generate comprehensive patient insights: {str(e)}")
            
    async def _run_symptom_forecasting(self, 
                                    patient_id: UUID,
                                    patient_data: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            if "risk_alerts" in forecast:
                for alert in forecast["risk_alerts"]:
                    key_insights.append({
                        ...
                        "importance": alert.get("importance", 0) + 0.5  # Prioritize risk alerts
                    })
    ...
    async def _run_biometric_correlation(self, 
                                      patient_id: UUID,
                                      patient_data: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            if "anomalies" in correlation:
                for anomaly in correlation["anomalies"]:
                    key_insights.append({
                        ...
                        "importance": anomaly.get("severity", 0) + 0.3  # Prioritize anomalies
                    })
    ...
    async def _run_pharmacogenomics(self, 
                                 patient_id: UUID,
                                 patient_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            medication_responses = await self.pharmacogenomics_service.predict_medication_responses(
                patient_id=patient_id,
            ...
            treatment_plan = None
            if diagnosis:
                treatment_plan = await self.pharmacogenomics_service.recommend_treatment_plan(
                    patient_id=patient_id,
                    patient_data=patient_data,
                    diagnosis=diagnosis,
            ...
            if "recommendations" in gene_interactions:
                for rec in gene_interactions["recommendations"][:2]:  # Top 2
                    key_insights.append({
                        ...
                        "importance": rec.get("importance", 0) + 0.2  # Prioritize gene interactions
                    })
                    
            # Add treatment plan summary
            if treatment_plan and "recommendations" in treatment_plan and "summary" in treatment_plan["recommendations"]:
                for rec in treatment_plan["recommendations"]["summary"][:3]:  # Top 3
                    key_insights.append({
                        "source": "pharmacogenomics",
                        "type": "treatment_plan",
                        "insight": rec.get("recommendation_text", ""),
                        "importance": rec.get("importance", 0) + 0.1  # Slight priority for treatment plan
                    })
    ...
    async def _generate_integrated_recommendations(self, 
        ...
        if len(integrated_recommendations) >= 2:
            ...
            symptom_insights = [i for i in all_insights if i.get("source") == "symptom_forecasting"]
            pharma_insights = [i for i in all_insights if i.get("source") == "pharmacogenomics"]
            ...
            biometric_insights = [i for i in all_insights if i.get("source") == "biometric_correlation"]
        ...
        # Sort by importance
        integrated_recommendations.sort(key=lambda x: x.get("importance", 0), reverse=True)
    ...
    async def update_digital_twin(self, 
        ...
        try:
            ...
            if "genetic_markers" in patient_data and "medication_responses" in patient_data:
                # This would be implemented in a real system, but for now we'll just log it
                logging.info(f"Pharmacogenomics model update for patient {patient_id} would happen here")
    ...
    async def get_digital_twin_status(self, patient_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            symptom_status = await self.symptom_forecasting_service.get_model_status(patient_id)
            biometric_status = await self.biometric_correlation_service.get_model_status(patient_id)
