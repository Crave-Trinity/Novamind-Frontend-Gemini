...
class DataPreprocessor:
    ...
    def handle_missing_values(
        ...
        if is_pandas:
            ...
            imputed_data = pd.DataFrame(
                imputer.fit_transform(data), columns=column_names, index=index
            )
        else:
    ...
    def extract_temporal_features(
        timestamps: Union[List[datetime], pd.Series], include_cyclical: bool = True
    ) -> pd.DataFrame:
        """
            ...
            if method == "one-hot":
                ...
                if n_categories > max_categories:
                    ...
                    one_hot = pd.get_dummies(
                        encoded_data[col], prefix=col, drop_first=False
                    )
                    encoded_data = pd.concat(
                        [encoded_data.drop(col, axis=1), one_hot], axis=1
                    )
            elif method == "label":
                # Label encode
                encoded_data[col] = encoded_data[col].astype("category").cat.codes
    ...
    def prepare_biometric_data(
        ...
        if missing_fields:
            logger.warning(
                f"Missing required fields in biometric data: {missing_fields}"
            )
    ...
    def prepare_symptom_data(
        symptom_data: List[Dict[str, Any]], required_fields: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
        ...
        # Default required fields
        if required_fields is None:
            required_fields = ["timestamp", "symptom_type", "severity", "duration"]
